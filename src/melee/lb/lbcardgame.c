...
#ifdef TARGET_PC
        struct SceneCameraDesc* cams =
            DP(struct SceneCameraDesc, _p(scene_data)->cameras);
...
#ifdef TARGET_PC
        DynamicModelDesc* model0 =
            DP(DynamicModelDesc, DP(DiscU32, _p(scene_data)->models)[0].v);
...
#ifdef TARGET_PC
        gm_8016895C(jobj, (void*) model0, 0);
#else
...
void lbCardGame_LoadArchive(int jobj_translate_idx)
{
    if (_p(icon_data) == NULL) {
        lbArchive_80016DBC("LbMcGame.", (void**) &_p(icon_data), "MemCardIconData", 0);
