...
{
    Item* ip = GET_ITEM(gobj);
    itOldottoseaAttributes* attrs = DP(itOldottoseaAttributes, ip->xC4_article_data->x4_specialAttributes);
    Item* ip2;
    f32 dx;
    f32 dy;
    f32 dz;
...
{
    Item* ip = GET_ITEM(gobj);
    itWhiteBeaAttributes* attr = DP(itWhiteBeaAttributes, ip->xC4_article_data->x4_specialAttributes);
    PAD_STACK(16);
