...
/* ponytail: on-disc view of the fields the decomp reads through
 * itStar_ItemVars (a runtime union member, so it cannot be DISC_STRUCT). */
...
DISC_ASSERT_SIZE(itStarAttributes, 0x18);
...
void itStar_Logic10_Spawned(Item_GObj* item_gobj)
{
    ...
    itStarAttributes* star = DP(itStarAttributes, item->xC4_article_data->x4_specialAttributes);
...
bool itStar_UnkMotion0_Coll(Item_GObj* item_gobj)
{
    ...
    if (it_8026DA08(item_gobj)) {
        ...
        itStarAttributes* star = DP(itStarAttributes, item2->xC4_article_data->x4_specialAttributes);
