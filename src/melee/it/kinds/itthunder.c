...
void itThunder_Logic7_Spawned(Item_GObj* gobj)
{
    ...
    itThunderPokemonAttributes* attrs = DP(itThunderPokemonAttributes, ip->xC4_article_data->x4_specialAttributes);
    ip->facing_dir = 0.0f;
...
void itThunder_UnkMotion2_Phys(Item_GObj* gobj)
{
    ...
    if (item->xDAC_itcmd_var0 != 0) {
        itThunderPokemonAttributes* attrs = DP(itThunderPokemonAttributes, item->xC4_article_data->x4_specialAttributes);
