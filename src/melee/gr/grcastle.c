...
void grCastle_801CE3AC(Ground_GObj* gobj)
{
    ...
    if (gp->u.castle11.xCC != NULL) {
        Quaternion rot;
        Vec3 pos;
        Vec3 offset;

        jobj = HSD_GObjGetHSDObj(gp->u.castle11.xCC);
        rot = grCs_803B7EB8;
...
void grCastle_801CE578(Ground_GObj* gobj)
{
    ...
    {
        CmSubject* cam = gp->u.castle11.xD8;
        if (cam != NULL) {
    ...
    if (!gp->u.castle11.xC4.b0) {
        ...
        if (grAnime_801C83D0(gobj, 0, 1)) {
            ...
            if (gp->u.castle11.xCC != NULL) {
                Ground_801C4A08(gp->u.castle11.xCC);
            }
...
void grCastle_801CE8E8(Ground_GObj* gobj)
{
    ...
    gp2 = GET_GROUND(gobj);
    gp2->u.castle1.xC8 = Camera_80029044(2);
    subject = gp2->u.castle1.xC8;
    if (subject != NULL) {
...
void grCastle_801CE9E8(Ground_GObj* gobj)
{
    ...
    Ground* tmp;
    Ground* gp = GET_GROUND(gobj);
    CmSubject* subject = gp->u.castle1.xC8;
    PAD_STACK(8);
    if (subject != NULL) {
        lb_8000B1CC(Ground_801C3FA4(gobj, 0), NULL, &pos);
        subject->pos = pos;
    ...
    if (grAnime_801C83D0(gobj, 0, 1)) {
        HSD_GObj* mat = (tmp = gp)->u.castle1.xC4;
        if (mat != NULL) {
            grMaterial_801C8CDC(mat);
        }
        gp->u.castle1.xC4 = NULL;
        {
            Ground* gp2 = gobj->user_data;
            CmSubject* subj2 = (tmp = gp2)->u.castle1.xC8;
            if (subj2 != NULL) {
                Camera_800290D4(subj2);
                gp2->u.castle1.xC8 = NULL;
            }
        }
        Ground_801C4A08(gobj);
    }
...
void grCastle_801CF308(Ground_GObj* gobj)
{
    ...
    case 2: {
        ...
        if (delay > 0) {
            ...
            if (val != -1) {
                ...
                gp->u.castle5.xC4 = 3;
                gp->u.castle7.xD8 = (HSD_GObj*) grMaterial_801C8CFC(
                    0, 1, gp, jobj, NULL,
    ...
    if (var_r6 != 0) {
        lb_8000B1CC(gp->u.castle7.xD4, NULL, &pos);
        HSD_JObjSetTranslate(jobj, &pos);

        HSD_JObjGetRotation(gp->u.castle7.xD4, &quat);
...

    {
        ...
        if (entity != NULL) {
            sat_gp->u.castle7.xD4 =
                Ground_801C3FA4(entity, (s32) targets.e[idx].jobj_idx);
