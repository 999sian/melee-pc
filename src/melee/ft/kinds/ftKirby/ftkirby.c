...
void ftKb_Init_OnLoad(HSD_GObj* gobj)
{
    ...
    it_8026B3F8(DP(Article, item_list[0].v), It_Kind_Kirby_CBeam);
    it_8026B3F8(DP(Article, item_list[1].v), It_Kind_Kirby_Hammer);
    it_8026B3F8(DP(Article, item_list[2].v), It_Kind_Unk1);
    it_8026B3F8(DP(Article, item_list[3].v), It_Kind_Unk2);
}
...
{
    *arg1 = DP(ArticleDynamicBones,
               DP(ftDynamics, ft_80459B88.hats[Ft_Kind_Pichu]->hat_dynamics[4])
                   ->ftDynamicBones);
}

void ftKb_Init_800EEB1C(Fighter_GObj* gobj, s32* arg1)
{
    *arg1 = DP(ftDynamics, ft_80459B88.hats[Ft_Kind_Pichu]->hat_dynamics[4])->x4;
}
...
void ftKb_SpecialN_800EF040(Fighter_GObj* gobj, int arg1, KirbyHatStruct* hat)
{
    ...
    if (mask != 0) {
        Fighter* fp = GET_FIGHTER(gobj);
        struct Fighter_804D6540_t* ft_data =
            DP(struct Fighter_804D6540_t, Fighter_804D6540[fp->kind].v);
        int count = ft_data->x4;
        HSD_Joint* joint = ftKb_Init_803C9FC8[arg1][fp->x619_costume_id].joint;
        struct Fighter_804D6540_x0_t* parts =
            DP(struct Fighter_804D6540_x0_t, ft_data->x0);
        int i;
...
void ftKb_SpecialN_800EF0E4(Fighter_GObj* gobj, int arg1, u8* arg2)
{
    ...
    while (current_joint != NULL) {
        group_count = 0;
        ftKb_SpecialN_800EF0E4_find_bone(fp, &insert_part_idx, &arg2_idx,
                                         &jobj);
        dobj = HSD_DObjLoadDesc(DP(HSD_DObjDesc, current_joint->u.dobjdesc));
        arg2[arg2_idx] = total_dobjs;
        if (dobj != NULL) {
            tail = HSD_JObjGetDObj(jobj);
            HSD_DObjResolveRefsAll(
                dobj, DP(HSD_DObjDesc, current_joint->u.dobjdesc));
            if (tail == NULL) {
...
void ftKb_SpecialN_800EF35C(Fighter_GObj* gobj, int arg1, u8* arg2)
{
    ...
    while (matanimjoint != NULL) {
        if (ftParts_8007506C(fp->kind, arg1) != 0) {
            ...
            if (matanimjoint->matanim != 0) {
                HSD_DObjAddAnimAll(fp->u.kb.hat.x14.data[*arg2],
                                   DP(HSD_MatAnim, matanimjoint->matanim),
                                   NULL);
            }
            arg1++;
            arg2++;
...
void ftKb_SpecialN_800EF438(Fighter_GObj* gobj, KirbyHatStruct* hat)
{
    ...
    HSD_JObj* jobj;
    HSD_Joint* root =
        (HSD_Joint*) (jobj = (HSD_JObj*) DP(void, hat->hat_dynamics[2]));
    s32 byte_off;
    Fighter* fp = GET_FIGHTER(gobj);
    s32 total_dobjs;
    s32 insert_part_idx;
    ...
    if (root != NULL) {
        s32 byte_base;
        ftPartsPObjSetDefaultClass();
        ftKb_SpecialN_insert_joint_refs(&total_dobjs, root, fp,
        ...
        while (current_joint != NULL) {
            ...
            dobj =
                HSD_DObjLoadDesc(DP(HSD_DObjDesc, current_joint->u.dobjdesc));
            if (dobj != NULL) {
                tail = HSD_JObjGetDObj(jobj);
        ...
        fp->u.kb.hat.x1C.count = total_dobjs;
        ftPartsPObjClearDefaultClass();
    } else {
        fp->u.kb.hat.x1C.data[0] = NULL;
    }
}
...
{
    Fighter* fp = GET_FIGHTER(gobj);
    struct Fighter_804D6540_t* tbl =
        DP(struct Fighter_804D6540_t, Fighter_804D6540[fp->kind].v);
    int i = tbl->x4 - 1;
    for (; i >= 0; i--) {
        if ((1 << i) & mask) {
            ftParts_800755E8(
                fp, &DP(struct Fighter_804D6540_x0_t, tbl->x0)[i]);
        }
    }
}
...
void ftKb_SpecialN_800EF69C(Fighter_GObj* gobj, int arg1, KirbyHatStruct* hat)
{
    ...
    if (fp->u.kb.hat.x14.data != NULL) {
        int i;
        for (i = 0;
             i < DP(FighterPartsTable, ftPartsTable[fp->kind].v)->parts_num;
             i++) {
...
void ftKb_SpecialN_800EFA40(HSD_GObj* gobj)
{
    ...
    if (fp->u.kb.hat.jobj == NULL) {
        ...
        ftPartsPObjSetDefaultClass();
        fp->u.kb.hat.jobj = HSD_JObjLoadJoint(DP(HSD_Joint, unk->x0));
        fp->x2225_b2 = true;
        ftPartsPObjClearDefaultClass();
                                ...
                                KirbyHatStruct* hat)
{
    fp->u.kb.hat.x14.data = HSD_ObjAlloc(&fighter_x2040_alloc_data);
    ftPartsPObjSetDefaultClass();
    fp->u.kb.hat.jobj = HSD_JObjLoadJoint(DP(HSD_Joint, hat->hat_joint));
    fp->x2225_b2 = true;
    ftPartsPObjClearDefaultClass();
    ftParts_80075650(gobj, fp->u.kb.hat.jobj, &fp->u.kb.hat.x14);
...
#define LOAD_HAT(gobj, fp, fp2, kind, hat, part_dobj_indices)                 \
    do {                                                                      \
        ...
        ftParts_8007487C((FtPartsDesc*) (hat), &(fp)->u.kb.hat.x24,           \
                         ...
                         &(fp)->u.kb.hat.x1C);                                \
        ftAnim_80070200(fp, (ftData_x8_x8*) ((u8*) (hat) + 8),                 \
...
void ftKb_SpecialN_800F14B4(Fighter_GObj* gobj)
{
    ...
    lookup = DP(FtPartsVisLookup, hat->hat_dynamics[3]);
    fp->u.kb.hat.x24.xC[4] = lookup;
    fp->x5AC.xC[4] = lookup;
    ftParts_80074D7C(&fp->u.kb.hat.x24, 4, &fp->u.kb.hat.x14);
    ftKb_SpecialN_800F1420(gobj, (u32*) (DP(u8, hat->hat_dynamics[4]) + 4));
    *(u32*) &fp->x610_color_rgba[1] = *(u32*) (DP(u8, hat->hat_dynamics[4]) + 8);
...
void ftKb_SpecialN_800F16D0(Fighter_GObj* gobj, FighterKind kind)
{
    ...
    switch (kind) {
    case Ft_Kind_Mario:
        it_8026B3F8(DP(Article, g->x0->xC), It_Kind_Kirby_MarioFire);
        break;
    case Ft_Kind_Luigi:
        it_8026B3F8(DP(Article, g->hats[16]->hat_dynamics[0]),
                    It_Kind_Kirby_LuigiFire);
        break;
    case Ft_Kind_Popo:
        it_8026B3F8(DP(Article, g->hats[9]->hat_dynamics[0]),
                    It_Kind_Kirby_IceClimberIce);
        break;
    case Ft_Kind_Peach:
        hat = g->hats[8];
        it_8026B3F8(DP(Article, hat->hat_dynamics[0]), It_Kind_Kirby_PeachToad);
        it_8026B3F8(DP(Article, hat->hat_dynamics[1]),
                    It_Kind_Kirby_PeachToadSpore);
        break;
    case Ft_Kind_Fox:
        hat = g->hats[0];
        it_8026B3F8(DP(Article, hat->hat_dynamics[0]), It_Kind_Kirby_FoxLaser);
        it_8026B3F8(DP(Article, hat->hat_dynamics[1]), It_Kind_Kirby_FoxBlaster);
        break;
    case Ft_Kind_Falco:
        hat = g->hats[21];
        it_8026B3F8(DP(Article, hat->hat_dynamics[3]), It_Kind_Kirby_FalcoLaser);
        it_8026B3F8(DP(Article, hat->hat_dynamics[4]),
                    It_Kind_Kirby_FalcoBlaster);
        break;
    case Ft_Kind_Link:
        hat = g->hats[5];
        it_8026B3F8(DP(Article, hat->hat_dynamics[0]), It_Kind_Kirby_LinkArrow);
        it_8026B3F8(DP(Article, hat->hat_dynamics[1]), It_Kind_Kirby_LinkBow);
        break;
    case Ft_Kind_CLink:
        hat = g->hats[19];
        it_8026B3F8(DP(Article, hat->hat_dynamics[0]), It_Kind_Kirby_CLinkArrow);
        it_8026B3F8(DP(Article, hat->hat_dynamics[1]), It_Kind_Kirby_CLinkBow);
        break;
    case Ft_Kind_Mewtwo:
        it_8026B3F8(DP(Article, g->hats[15]->hat_dynamics[3]),
                    It_Kind_Kirby_MewtwoShadowBall);
        break;
    case Ft_Kind_Ness:
        hat = g->hats[7];
        it_8026B3F8(DP(Article, hat->hat_dynamics[0]),
    ...
    case Ft_Kind_Pichu:
        ...
        break;
    case Ft_Kind_Samus:
        it_8026B3F8(DP(Article, g->hats[12]->hat_dynamics[0]),
                    It_Kind_Kirby_SamusCharge);
        break;
    case Ft_Kind_Koopa:
        it_8026B3F8(DP(Article, g->hats[4]->hat_dynamics[0]),
                    It_Kind_Kirby_KoopaFlame);
        break;
    case Ft_Kind_GKoops:
        it_8026B3F8(DP(Article, g->hats[30]->hat_dynamics[0]),
                    It_Kind_Kirby_KoopaFlame);
        break;
    case Ft_Kind_Seak:
        hat = g->hats[6];
    ...
    case Ft_Kind_DrMario:
        it_8026B3F8(DP(Article, g->hats[20]->hat_dynamics[0]),
                    It_Kind_Kirby_DrMarioVitamin);
        break;
    case Ft_Kind_GameWatch:
        hat = g->hats[23];
        it_8026B3F8(DP(Article, hat->hat_dynamics[5]),
                    It_Kind_Kirby_GameWatchChef);
        it_8026B3F8(DP(Article, hat->hat_dynamics[6]),
                    It_Kind_Kirby_GameWatchChefPan);
        break;
    case Ft_Kind_Yoshi:
        it_8026B3F8(DP(Article, g->hats[13]->hat_dynamics[5]),
