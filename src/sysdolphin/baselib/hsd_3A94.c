#include "hsd_3A94.h"

#include "hsd_3B2B.h"
#include "hsd_3B2E.h"

#ifdef MUST_MATCH
#include <stddef.h>
#endif
#include <string.h>

#include <dolphin/card.h>
#include <dolphin/os.h>

typedef struct CardBufEntry {
    s32 x0, x4, x8, xC;
    s32 x10;
    s32 x14, x18, x1C, x20;
} CardBufEntry;

typedef struct CardBlock {
    /* 0x00 */ u8 pad_00[0x10];
    /* 0x10 */ u8 x10;
    /* 0x11 */ u8 x11;
    /* 0x12 */ u8 x12;
    /* 0x13 */ u8 pad_13[0x0D];
    /* 0x20 */ u8 x20[1];
} CardBlock;

typedef struct CardCmd {
    /* 0x00 */ s32 type;
    /* 0x04 */ u32 state; ///< CardState*, stored as u32 (see hsd_3A94.h)
    /* 0x08 */ s32 x8;
    /* 0x0C */ s32 xC;
    /* 0x10 */ s32 x10;
    /* 0x14 */ s32 x14;
    /* 0x18 */ u32 x18; ///< void*, stored as u32
    /* 0x1C */ s32 x1C;
    /* 0x20 */ s32 x20;
} CardCmd;

/// Stack staging slot for one queued command.
typedef union CardCmdBuf {
    CardCmd cmd;
    s32 words[10];
} CardCmdBuf;

#ifdef MUST_MATCH
typedef union CardCmdStorage {
    CardCmd command;
    s32 words[9];
} CardCmdStorage;

#define MATCH_CARD_CMD_FIELD(storage, member, adjustment)                     \
    storage.words[(offsetof(CardCmd, member) - adjustment) / sizeof(s32)]
#endif

typedef struct HsdCmdEntry {
    s32 type;
    s32 f1;
    s32 f2;
    s32 f3;
    s32 f4;
    s32 f5;
} HsdCmdEntry;

typedef struct CardContext {
    /* 0x0000 */ s32 x0;
    /* 0x0004 */ u32 x4; ///< CardState*, stored as u32
    /* 0x0008 */ u32 x8; ///< void (*)(s32, s32), stored as u32
    /* 0x000C */ s32 xC;
    /* 0x0010 */ CardCmd x10[128];
    /* 0x1210 */ HsdCmdEntry x1210[32];
} CardContext;

typedef struct CardQueueEntry {
    /* 0x00 */ s32 x0;
    /* 0x04 */ s32 x4;
    /* 0x08 */ s32 x8;
    /* 0x0C */ s32 xC;
    /* 0x10 */ s32 x10;
    /* 0x14 */ u32 x14; ///< void (*)(s32, s32), stored as u32
} CardQueueEntry;

STATIC_ASSERT(sizeof(CardCmd) == 0x24);
STATIC_ASSERT(sizeof(CardBufEntry) == 0x24);
STATIC_ASSERT(sizeof(HsdCmdEntry) == 0x18);
STATIC_ASSERT(sizeof(CardQueueEntry) == 0x18);
STATIC_ASSERT(sizeof(CardContext) == 0x1510);

#define CMD_S32(off)                                                          \
    (((CardBufEntry*) ((unsigned char*) op + (off)))[hsd_804D7980].x0)
#define CMD_STATE U32_TO_PTR(CardState*, CMD_S32(0x14))
#define CMD_PTR(off) U32_TO_PTR(void*, CMD_S32(off))
#define CMD_TYPE (op[hsd_804D7980].x10)
#define CARD_QUEUE_CMD(cmd) fn_803AC168((cmd))
#define CARD_WRITE_BLOCK(state, block_idx, file_id, seq_num, payload,         \
                         payload_len, file_idx)                               \
    fn_803ACFC0((state), (block_idx), (file_id), (seq_num), (payload),        \
                (payload_len), (file_idx))

/* 3A949C */ static void hsd_803A949C(s32 chan, s32 arg1);
/* 3ACB74 */ static s32 fn_803ACB74(s32 seq_a, s32 seq_b);
/* 4D7980 */ extern volatile s32 hsd_804D7980;
/* 4D7984 */ extern volatile s32 hsd_804D7984;
/* 4D7988 */ extern s32 hsd_804D7988;
/* 4D798C */ extern s32 hsd_804D798C;
/* 4D7998 */ extern s32 hsd_804D7998;
/* 4D799C */ extern s32 hsd_804D799C;
/// .sbss globals emit in reverse declaration order.
/* 4D79C8 */ u8 hsd_804D79C8;
/* 4D79C4 */ s32 hsd_804D79C4;
/* 4D79C0 */ s32 hsd_804D79C0;
/* 4D79BC */ u8* hsd_804D79BC;
/* 4D79B8 */ u8* hsd_804D79B8;
/* 4D79B0 */ u8 hsd_804D79B0[8];
/* 4D79AC */ s32 hsd_804D79AC;
/* 4D79A8 */ s32 hsd_804D79A8;
/* 4D79A4 */ u8* hsd_804D79A4;
/* 4D79A0 */ u8* hsd_804D79A0;

static inline s32 hsd_803A949C_Close(CardState* state)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDClose(&state->file_info);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}

void hsd_803A949C(s32 chan, s32 arg1)
{
    CardBufEntry* op = (CardBufEntry*) hsd_804D1138;
    CardState* state;
    u8* block;
    s32 result;
    s32 icon_size;
    s32 remaining;
    s32 remaining11;
    s32 offset;
    s32 offset13;
    s32 data_offset;
    s32 hdr_plus_icon;
    s32 slot;
    s32 i;

    if (hsd_804D799C != 1) {
        return;
    }

    state = CMD_STATE;

    switch (CMD_TYPE) {
    case 2:
        if (arg1 != 0) {
            hsd_803A949C_Close(state);
            hsd_804D7988 = arg1;
            break;
        }

        if (CMD_S32(0x20) < 0) {
            if (hsd_803A949C_Close(state) < 0) {
                hsd_804D7988 = arg1;
            }
            break;
        }

        if (CMD_S32(0x20) == 0) {
            offset = (state->x24 + 0x30) % state->x8;
            if (CMD_S32(0x30) > 0) {
                if (hsd_803B31CC(CARD_BUF(state) + offset, state->x8 - offset) < 0) {
                    hsd_803A949C_Close(state);
                    hsd_804D7988 = -0x105;
                    break;
                }
                if (CMD_PTR(0x28) != NULL) {
                    u8* src = CARD_BUF(state) + offset;
                    memcpy(CMD_PTR(0x28), src + 0x20, CMD_S32(0x30));
                }
            }
            result = hsd_803A949C_Close(state);
        } else {
            if (hsd_803B31CC(CARD_BUF(state), state->x8) < 0) {
                hsd_803A949C_Close(state);
                hsd_804D7988 = -0x105;
                break;
            }
            if (CMD_S32(0x30) > 0 && CMD_PTR(0x28) != NULL) {
                memcpy(CMD_PTR(0x28), CARD_BUF(state) + 0x20, CMD_S32(0x30));
            }
            result = hsd_803A949C_Close(state);
        }
        if (result < 0) {
            hsd_804D7988 = result;
        }
        break;

    case 15:
        if (arg1 != 0) {
            hsd_803A949C_Close(state);
            hsd_804D7988 = arg1;
        } else if (hsd_803B31CC(CARD_BUF(state), state->x8) < 0) {
            hsd_803A949C_Close(state);
            hsd_804D7988 = -0x105;
        } else {
            result = hsd_803A949C_Close(state);
            if (result < 0) {
                hsd_804D7988 = result;
            }
        }
        break;

    case 5:
        if (arg1 != 0) {
            hsd_803A949C_Close(state);
            hsd_804D7988 = arg1;
            break;
        }

        if (CMD_S32(0x20) == 0) {
            if (CMD_S32(0x30) <= 0) {
                result = hsd_803A949C_Close(state);
                if (result < 0) {
                    hsd_804D7988 = result;
                }
                break;
            }

            offset = (state->x24 + 0x30) % state->x8;
            if (hsd_803B31CC(CARD_BUF(state) + offset, state->x8 - offset) < 0) {
                hsd_803A949C_Close(state);
                hsd_804D7988 = 2;
                break;
            }
            result = hsd_803A949C_Close(state);
            if (result < 0) {
                hsd_804D7988 = result;
                break;
            }
            result = hsd_804D7980;
            block = CARD_BUF(state) + offset;
            result = ((CardBufEntry*) ((u8*) op + 0x20))[result].x0;
            if (((block[0x10] << 8) | block[0x11]) != result) {
                hsd_804D7988 = 2;
            } else if ((s32) block[0x12] != CMD_S32(0x24)) {
                hsd_804D7988 = 2;
            } else if (CMD_S32(0x30) > 0 &&
                       memcmp(CMD_PTR(0x28), (block += 0x20),
                              CMD_S32(0x30)) != 0)
            {
                hsd_804D7988 = 2;
            }
        } else {
            if (hsd_803B31CC(CARD_BUF(state), state->x8) < 0) {
                hsd_803A949C_Close(state);
                hsd_804D7988 = 2;
                break;
            }
            result = hsd_803A949C_Close(state);
            if (result < 0) {
                hsd_804D7988 = result;
                break;
            }
            result = (((CardBlock*) CARD_BUF(state))->x10 << 8) |
                     ((CardBlock*) CARD_BUF(state))->x11;
            block = CARD_BUF(state);
            if (result != CMD_S32(0x20)) {
                hsd_804D7988 = 2;
            } else if ((s32) block[0x12] != CMD_S32(0x24)) {
                hsd_804D7988 = 2;
            } else if (CMD_S32(0x30) > 0 &&
                       memcmp(CMD_PTR(0x28), block + 0x20,
                              CMD_S32(0x30)) != 0)
            {
                hsd_804D7988 = 2;
            }
        }
        break;

    case 10:
        if (arg1 != 0) {
            hsd_803A949C_Close(state);
            hsd_804D7988 = arg1;
            break;
        }

        result = hsd_803A949C_Close(state);
        if (result < 0) {
            hsd_804D7988 = result;
            break;
        }

        switch (state->x3B0) {
        case 2:
            icon_size = 0x1800;
            break;
        case 1:
            icon_size = 0xE00;
            break;
        default:
            icon_size = 0;
            break;
        }

        if (CMD_S32(0x18) == 0) {
            if (memcmp(CARD_BUF(state), state->x370, 0x40) != 0) {
                hsd_804D7988 = 2;
                break;
            }
            if (icon_size > 0 && memcmp(CARD_BUF(state) + 0x40,
                                        CMD_PTR(0x1c), icon_size) != 0)
            {
                hsd_804D7988 = 2;
                break;
            }
            hdr_plus_icon = icon_size + 0x40;
            if (state->x24 > state->x8) {
                if (memcmp(CARD_BUF(state) + hdr_plus_icon, CMD_PTR(0x20),
                           state->x8 - hdr_plus_icon) != 0)
                {
                    hsd_804D7988 = 2;
                    break;
                }
                hsd_803B2B20(CARD_BUF(CMD_STATE), CMD_STATE->x8,
                             &CMD_STATE->digest[CMD_S32(0x18) * 0x10]);
            } else if (memcmp(CARD_BUF(state) + hdr_plus_icon, CMD_PTR(0x20),
                              state->x24 - hdr_plus_icon) != 0)
            {
                hsd_804D7988 = 2;
            } else {
                hsd_803B2B20(CARD_BUF(CMD_STATE), CMD_STATE->x24,
                             &CMD_STATE->digest[CMD_S32(0x18) * 0x10]);
                if (memcmp(CARD_BUF(state) + state->x24, CMD_STATE->digest, 0x30) !=
                    0)
                {
                    hsd_804D7988 = 2;
                }
            }
        } else {
            data_offset = (state->x8 * CMD_S32(0x18)) - 0x40 - icon_size;
            remaining = state->x24 - state->x8 * CMD_S32(0x18);
            if ((u32) remaining > state->x8) {
                if (memcmp(CARD_BUF(state), (u8*) CMD_PTR(0x20) + data_offset,
                           state->x8) != 0)
                {
                    hsd_804D7988 = 2;
                    break;
                }
                hsd_803B2B20(CARD_BUF(CMD_STATE), CMD_STATE->x8,
                             &CMD_STATE->digest[CMD_S32(0x18) * 0x10]);
            } else if (memcmp(CARD_BUF(state), (u8*) CMD_PTR(0x20) + data_offset,
                              remaining) != 0)
            {
                hsd_804D7988 = 2;
            } else {
                hsd_803B2B20(CARD_BUF(CMD_STATE), remaining,
                             &CMD_STATE->digest[CMD_S32(0x18) * 0x10]);
                if (memcmp(CARD_BUF(state) + remaining, CMD_STATE->digest, 0x30) !=
                    0)
                {
                    hsd_804D7988 = 2;
                }
            }
        }
        break;

    case 11:
        if (arg1 != 0) {
            hsd_803A949C_Close(state);
            hsd_804D7988 = arg1;
            break;
        }

        result = hsd_803A949C_Close(state);
        if (result < 0) {
            hsd_804D7988 = result;
            break;
        }

        switch (state->x3B0) {
        case 2:
            remaining11 = 0x1800;
            break;
        case 1:
            remaining11 = 0xE00;
            break;
        default:
            remaining11 = 0;
            break;
        }

        if (CMD_S32(0x18) == 0) {
            if (CMD_PTR(0x1c) != NULL) {
                memcpy(CMD_PTR(0x1c), CARD_BUF(state), 0x40);
            }
            if (remaining11 > 0 && CMD_PTR(0x20) != NULL) {
                memcpy(CMD_PTR(0x20), CARD_BUF(state) + 0x40, remaining11);
            }
            hdr_plus_icon = remaining11 + 0x40;
            if (state->x24 > state->x8) {
                if (CMD_PTR(0x24) != NULL) {
                    memcpy(CMD_PTR(0x24), CARD_BUF(state) + hdr_plus_icon,
                           state->x8 - hdr_plus_icon);
                }
                hsd_803B2B20(CARD_BUF(CMD_STATE), CMD_STATE->x8,
                             &CMD_STATE->digest[CMD_S32(0x18) * 0x10]);
            } else {
                if (CMD_PTR(0x24) != NULL) {
                    memcpy(CMD_PTR(0x24), CARD_BUF(state) + hdr_plus_icon,
                           state->x24 - hdr_plus_icon);
                }
                hsd_803B2B20(CARD_BUF(CMD_STATE), CMD_STATE->x24,
                             &CMD_STATE->digest[CMD_S32(0x18) * 0x10]);
                if (memcmp(CARD_BUF(state) + state->x24, CMD_STATE->digest, 0x30) !=
                    0)
                {
                    hsd_804D7988 = -0x107;
                }
            }
        } else {
            data_offset = (state->x8 * CMD_S32(0x18)) - 0x40 - remaining11;
            chan = state->x24 - state->x8 * CMD_S32(0x18);
            if ((u32) chan > state->x8) {
                if (CMD_PTR(0x24) != NULL) {
                    memcpy((u8*) CMD_PTR(0x24) + data_offset, CARD_BUF(state),
                           state->x8);
                }
                hsd_803B2B20(CARD_BUF(CMD_STATE), CMD_STATE->x8,
                             &CMD_STATE->digest[CMD_S32(0x18) * 0x10]);
            } else {
                if (CMD_PTR(0x24) != NULL) {
                    memcpy((u8*) CMD_PTR(0x24) + data_offset, CARD_BUF(state), chan);
                }
                hsd_803B2B20(CARD_BUF(CMD_STATE), chan,
                             &CMD_STATE->digest[CMD_S32(0x18) * 0x10]);
                if (memcmp(CARD_BUF(state) + chan, CMD_STATE->digest, 0x30) != 0) {
                    hsd_804D7988 = -0x107;
                }
            }
        }
        break;

    case 1:
        if (arg1 != 0) {
            state->x170[CMD_S32(0x1c)] = -0x7FFF;
            state->x270[CMD_S32(0x1c)] = 0;
            hsd_803A949C_Close(state);
            hsd_804D7988 = arg1;
            break;
        }
        if (CMD_S32(0x20) != 0xFFFF) {
            state->x170[CMD_S32(0x1c)] = CMD_S32(0x20);
            state->x270[CMD_S32(0x1c)] = CMD_S32(0x24);
        } else {
            state->x170[CMD_S32(0x1c)] = -0x7FFF;
            state->x270[CMD_S32(0x1c)] = 0;
        }
        result = hsd_803A949C_Close(state);
        if (result < 0) {
            hsd_804D7988 = result;
        }
        break;

    case 16:
        if (arg1 != 0) {
            state->x170[CMD_S32(0x1c)] = -0x7FFF;
            state->x270[CMD_S32(0x1c)] = 0;
            hsd_803A949C_Close(state);
            hsd_804D7988 = arg1;
            break;
        }
        if (CMD_S32(0x20) != 0xFFFF) {
            state->x170[CMD_S32(0x1c)] = CMD_S32(0x20);
            state->x270[CMD_S32(0x1c)] = CMD_S32(0x24);
        } else {
            state->x170[CMD_S32(0x1c)] = -0x7FFF;
            state->x270[CMD_S32(0x1c)] = 0;
        }
        result = hsd_803A949C_Close(state);
        if (result < 0) {
            hsd_804D7988 = result;
        }
        break;

    case 7:
        if (arg1 != 0) {
            hsd_803A949C_Close(state);
            hsd_804D7988 = arg1;
            break;
        }
        i = 0;
        result = state->file_info.fileNo;
        do {
            if (result != -1) {
                break;
            }
            i++;
        } while (i < 10);
        state->x20 = result;
        if (state->x20 < 0) {
            hsd_804D7988 = state->x20;
        }
        result = hsd_803A949C_Close(state);
        if (result < 0) {
            hsd_804D7988 = result;
        }
        break;

    case 8:
        if (arg1 != 0) {
            hsd_804D7988 = arg1;
        }
        break;

    case 9:
        if (arg1 != 0) {
            hsd_803A949C_Close(state);
            hsd_804D7988 = arg1;
        } else {
            result = hsd_803A949C_Close(state);
            if (result < 0) {
                hsd_804D7988 = result;
            }
        }
        break;

    case 13:
        slot = CMD_S32(0x20);
        hsd_803A949C_Close(state);
        if (arg1 != 0) {
            state->x170[slot] = -0x7FFF;
            state->x270[slot] = 0;
            break;
        }

        if (slot == 0) {
            offset13 = (state->x24 + 0x30) % state->x8;
            if (hsd_803B31CC(CARD_BUF(state) + offset13, state->x8 - offset13) < 0) {
                state->x170[0] = -0x7FFF;
                state->x270[0] = 0;
                break;
            }
            state->x170[slot] =
                CARD_BUF(state)[offset13 + 0x11] + (CARD_BUF(state)[offset13 + 0x10] << 8);
            if (state->x170[slot] != 0) {
                state->x170[slot] = -0x7FFF;
                state->x270[slot] = 0;
            } else {
                state->x270[slot] = CARD_BUF(state)[offset13 + 0x12];
                {
                    u8* src13 = CARD_BUF(state) + offset13;
                    hsd_803AC558(state, src13 + 0x13);
                }
            }
        } else {
            if (hsd_803B31CC(CARD_BUF(state), state->x8) < 0) {
                state->x170[slot] = -0x7FFF;
                state->x270[slot] = 0;
                break;
            }
            state->x170[slot] = CARD_BUF(state)[0x11] + (CARD_BUF(state)[0x10] << 8);
            if (state->x170[slot] < 0 || state->x170[slot] > state->x460) {
                state->x170[slot] = -0x7FFF;
                state->x270[slot] = 0;
            } else {
                state->x270[slot] = CARD_BUF(state)[0x12];
                hsd_803AC558(state, CARD_BUF(state) + 0x13);
            }
        }
        break;
    }

    CMD_TYPE = 0;
    hsd_804D799C = 0;
    hsd_804D7980 = (hsd_804D7980 + 1) % 128;
}

#define CMD_QUEUE(base) ((HsdCmdEntry*) ((base) + 0x1210))

s32 fn_803AA790(void)
{
    CardQueueEntry* entry;
    s32 result;
    s32 arg0;
    CardState* state;
    void (*callback)(s32, s32);

    entry = &((CardQueueEntry*) &hsd_804D2348)[hsd_804D7990];
    arg0 = entry->x4;
    state = U32_TO_PTR(CardState*, entry->x4);
    callback = U32_TO_PTR(void (*)(s32, s32), entry->x14);
    hsd_804D7990 = (hsd_804D7990 + 1) % 32;

    switch (entry->x0) {
    case 1:
        result = fn_803ADF90(state, entry->x8, U32_TO_PTR(u8*, entry->xC), 1,
                             callback);
        if (result < 0) {
            if (callback != NULL) {
                callback(entry->x8, result);
            }
        }
        entry->x0 = 0;
        return result;
    case 2:
        switch (state->x28[entry->x8]) {
        case 0:
            result = fn_803AE7F8(state, entry->x8, entry->xC, 1,
                                 (s32) entry->x14);
            break;
        case 1:
        case 2:
            result = fn_803AF3F0(state, entry->x8, entry->xC, 1,
                                 (s32) entry->x14);
            break;
        case 3:
            result = fn_803B0120(state, entry->x8, entry->xC, 1,
                                 (s32) entry->x14);
            break;
        default:
            result = -0x101;
            break;
        }
        if (result < 0) {
            if (callback != NULL) {
                callback(entry->x8, result);
            }
        }
        entry->x0 = 0;
        return result;
    case 3:
        result = fn_803B1F78(state, entry->x8, entry->xC, entry->x10,
                             (s32) entry->x14);
        if (result < 0) {
            if (callback != NULL) {
                callback(0, result);
            }
        }
        entry->x0 = 0;
        return result;
    case 4:
        result =
            fn_803B21E8(entry->x4, entry->xC, entry->x10, (s32) entry->x14);
        if (result < 0) {
            if (callback != NULL) {
                callback(0, result);
            }
        }
        entry->x0 = 0;
        return result;
    case 5:
        result = fn_803ADE4C(arg0, entry->x8, (s32) entry->x14);
        if (result < 0) {
            if (callback != NULL) {
                callback(0, result);
            }
        }
        entry->x0 = 0;
        return result;
    case 6:
        result = fn_803B26CC(state, entry->x8, entry->xC, entry->x10, callback);
        if (result < 0) {
            if (callback != NULL) {
                callback(0, result);
            }
        }
        entry->x0 = 0;
        return result;
    default:
        return -0x101;
    }
}

/// @todo Fix (ideally delete) macros
#undef CMD_TYPE
#undef CMD_STATE

#define CMD_TYPE cmd[0]
#define CMD_STATE U32_TO_PTR(CardState*, cmd[1])
#define CMD_X8 cmd[2]
#define CMD_XC cmd[3]
#define CMD_X10 cmd[4]
#define CMD_X14 cmd[5]
#define CMD_X18 U32_TO_PTR(void*, cmd[6])
#define CMD_X1C cmd[7]
#define CMD_X20 cmd[8]

static inline s32 retryCardFastOpen(s32 chan, s32 file_no,
                                    CARDFileInfo* file_info)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDFastOpen(chan, file_no, file_info);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}
static inline s32 retryCardReadAsync(CARDFileInfo* file_info, void* buffer,
                                     s32 length, s32 offset,
                                     CARDCallback callback)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDReadAsync(file_info, buffer, length, offset, callback);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}
static inline s32 retryCardWriteAsync(CARDFileInfo* file_info, void* buffer,
                                      s32 length, s32 offset,
                                      CARDCallback callback)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDWriteAsync(file_info, buffer, length, offset, callback);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}
static inline s32 retryCardClose(CARDFileInfo* file_info)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDClose(file_info);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}
static inline s32 retryCardCreateAsync(s32 chan, char* filename, s32 size,
                                       CARDFileInfo* file_info,
                                       CARDCallback callback)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDCreateAsync(chan, filename, size, file_info, callback);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}
static inline s32 retryCardGetStatus(s32 chan, s32 file_no, CARDStat* stat)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDGetStatus(chan, file_no, stat);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}
static inline s32 retryCardSetStatusAsync(s32 chan, s32 file_no,
                                          CARDStat* stat,
                                          CARDCallback callback)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDSetStatusAsync(chan, file_no, stat, callback);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}
static inline int setupCardIcons(const s32* cmd)
{
    u32 bit;
    s32 k;

    k = (bit = 0);
    for (; k < 8 && CMD_STATE->icon_speed[(int) k] != 0;) {
        CMD_STATE->stat.iconFormat =
            (CMD_STATE->stat.iconFormat & ~(CARD_STAT_ICON_MASK << bit)) |
            (CMD_STATE->icon_format[(int) k] << bit);
        CMD_STATE->stat.iconSpeed =
            (CMD_STATE->stat.iconSpeed & ~(CARD_STAT_SPEED_MASK << bit)) |
            (CMD_STATE->icon_speed[k++] << bit);
        bit += 2;
    }
    return k;
}
static inline void unpackCardStat(const s32* cmd, CARDStat* stat)
{
    s32 k;

    CMD_STATE->x3B0 = stat->bannerFormat & 3;
    for (k = 0; k < 8; k++) {
        CMD_STATE->icon_format[k] =
            (stat->iconFormat >> (2 * k)) & CARD_STAT_ICON_MASK;
        CMD_STATE->icon_speed[k] =
            (stat->iconSpeed >> (2 * k)) & CARD_STAT_SPEED_MASK;
    }
    CMD_STATE->x24 = hsd_803AC340(&CMD_STATE->x3B0);
    {
        u32 used = CMD_STATE->x24 + CMD_STATE->x8;
        used += 0x2F;
        CMD_STATE->x460 = stat->length / CMD_STATE->x8 - used / CMD_STATE->x8;
    }
}
static inline void rollbackCardCommands(CardContext* context, s32 snap)
{
    s32 saved = snap;

    while (saved != hsd_804D7984) {
        ((CardBufEntry*) &context->x4)[saved].xC = 0;
        saved = (saved + 1) % 128;
    }
    hsd_804D7984 = snap;
}
static inline void initHeaderBlockCommand(CardCmd* buf, CardState* state,
                                          u32 block, s32 file_id, s32 seq_num,
                                          s32 version)
{
    buf->type = 11;
    buf->state = PTR_TO_U32(state);
    buf->x8 = block;
    buf->xC = file_id;
    buf->x10 = seq_num;
    buf->x14 = version;
}
static inline s32 queueHeaderBlock(CardState* state, u32 block, s32 file_id,
                                   s32 seq_num, s32 version)
{
    CardCmd buf;

    initHeaderBlockCommand(&buf, state, block, file_id, seq_num, version);
    return fn_803AC168((s32*) &buf);
}
static inline s32 queueHeaderBlocks(CardState* state, s32 file_id, s32 seq_num,
                                    s32 version)
{
    u32 i;
    int result;

    for (i = 0; i < (0x2F + state->x24 + state->x8) / state->x8; i++) {
        result = queueHeaderBlock(state, i, file_id, seq_num, version);
        if (result < 0) {
            return result;
        }
    }
    return 0;
}

void hsd_803AAA48(void)
{
    s32 r;
    s32 chan;
    while (1) {
        CardContext* ctx = (CardContext*) hsd_804D1138;
        s32* cmd;
        s32 type;
        BOOL intr = OSDisableInterrupts();
        BOOL intr2;
        s32 busy;
        if (hsd_804D799C == 1) {
            busy = 1;
        } else {
            busy = 0;
        }
        OSRestoreInterrupts(intr);
        if (busy) {
            return;
        }

        if (hsd_804D7988 < 0) {
            while (((CardBufEntry*) hsd_804D1138)[hsd_804D7980].x10 != 0) {
                ((CardBufEntry*) hsd_804D1138)[hsd_804D7980].x10 = 0;
                hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            }
        }

        cmd = (s32*) &((CardBufEntry*) ctx)[hsd_804D7980];
        type = *(cmd += 4);

        switch ((u32) type) {
        case 0:
            if (ctx->x0 != 0) {
                if (ctx->x0 == 3) {
                    CardState* state = U32_TO_PTR(CardState*, ctx->x4);
                    s32 file = ctx->xC;
                    s32 blocks_before = fn_803AC6B8(state, file);
                    s32 file_blocks = fn_803AC634(state, file);
                    s32 total = fn_803AC7DC(state);
                    s32 map[64];
                    s32 j;
                    for (j = 0; j < file_blocks; j++) {
                        map[j] = -1;
                    }
                    for (j = 1; j <= total; j++) {
                        s32 fidx = state->x170[j];
                        if (fidx >= 0) {
                            s32 logical = fidx - blocks_before;
                            if (logical >= 0 && logical < file_blocks) {
                                if (map[logical] < 0) {
                                    map[logical] = j;
                                } else if (fn_803ACB74(
                                               state->x270[map[logical]],
                                               state->x270[j]) < 0)
                                {
                                    state->x170[map[logical]] *= -1;
                                    map[logical] = j;
                                } else {
                                    state->x170[j] *= -1;
                                }
                            }
                        }
                    }
                }
                if (ctx->x8 != 0) {
                    U32_TO_PTR(void (*)(s32, s32), ctx->x8)
                    (ctx->xC, hsd_804D7988);
                }
                ctx->x0 = 0;
            }
            hsd_804D7988 = 0;
            hsd_804D799C = 2;
            if (hsd_804D7990 == hsd_804D7994 &&
                CMD_QUEUE((u8*) ctx)[hsd_804D7990].type == 0)
            {
                return;
            }
            if (fn_803AA790() < 0) {
                return;
            }
            hsd_804D799C = 0;
            continue;
        case 5:
            if (hsd_804D7988 == 2) {
                CMD_TYPE = 0;
                hsd_804D7980 = (hsd_804D7980 + 1) % 128;
                continue;
            }
            r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                  &CMD_STATE->file_info);
            if (r < 0) {
                hsd_804D7988 = r;
                continue;
            }
            hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
            intr2 = OSDisableInterrupts();
            r = retryCardReadAsync(&CMD_STATE->file_info, CARD_BUF(CMD_STATE),
                                   CMD_STATE->x8, CMD_X1C,
                                   (void (*)(s32, s32)) hsd_803A949C);
            hsd_804D799C = 1;
            OSRestoreInterrupts(intr2);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                retryCardClose(&CMD_STATE->file_info);
                continue;
            }
            return;
        case 11:
            r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                  &CMD_STATE->file_info);
            if (r < 0) {
                hsd_804D7988 = r;
                continue;
            }
            hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
            if (CMD_X8 == 0) {
                memset(CMD_STATE->digest, 0, 0x30);
            }
            intr2 = OSDisableInterrupts();
            r = retryCardReadAsync(&CMD_STATE->file_info, CARD_BUF(CMD_STATE),
                                   CMD_STATE->x8, CMD_X8 * CMD_STATE->x8,
                                   (void (*)(s32, s32)) hsd_803A949C);
            hsd_804D799C = 1;
            OSRestoreInterrupts(intr2);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                retryCardClose(&CMD_STATE->file_info);
                continue;
            }
            return;
        case 10:
            if (hsd_804D7988 == 2) {
                CMD_TYPE = 0;
                hsd_804D7980 = (hsd_804D7980 + 1) % 128;
                continue;
            }
            r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                  &CMD_STATE->file_info);
            if (r < 0) {
                hsd_804D7988 = r;
                continue;
            }
            hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
            if (CMD_X8 == 0) {
                memset(CMD_STATE->digest, 0, 0x30);
            }
            intr2 = OSDisableInterrupts();
            r = retryCardReadAsync(&CMD_STATE->file_info, CARD_BUF(CMD_STATE),
                                   CMD_STATE->x8, CMD_X8 * CMD_STATE->x8,
                                   (void (*)(s32, s32)) hsd_803A949C);
            hsd_804D799C = 1;
            OSRestoreInterrupts(intr2);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                retryCardClose(&CMD_STATE->file_info);
                continue;
            }
            return;
        case 6:
            if (hsd_804D7988 == 2) {
                hsd_804D7988 = 0;
                CMD_TYPE = 0;
                hsd_804D7980 = (hsd_804D7980 + 1) % 128;
                continue;
            } else {
                hsd_804D7988 = 1;
                CMD_TYPE = 0;
                hsd_804D7980 = (hsd_804D7980 + 1) % 128;
                continue;
            }
        case 2:
            if (hsd_804D7988 != 1) {
                r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                      &CMD_STATE->file_info);
                if (r < 0) {
                    hsd_804D7988 = r;
                    continue;
                }
                hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
                intr2 = OSDisableInterrupts();
                r = retryCardReadAsync(&CMD_STATE->file_info, CARD_BUF(CMD_STATE),
                                       CMD_STATE->x8, CMD_X1C,
                                       (void (*)(s32, s32)) hsd_803A949C);
                hsd_804D799C = 1;
                OSRestoreInterrupts(intr2);
                if (r < 0) {
                    hsd_804D7988 = r;
                    hsd_804D799C = 0;
                    retryCardClose(&CMD_STATE->file_info);
                    continue;
                }
                return;
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 15:
            if (hsd_804D7988 != 1) {
                r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                      &CMD_STATE->file_info);
                if (r < 0) {
                    hsd_804D7988 = r;
                    continue;
                }
                hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
                intr2 = OSDisableInterrupts();
                r = retryCardReadAsync(&CMD_STATE->file_info, CARD_BUF(CMD_STATE),
                                       CMD_STATE->x8, CMD_X1C,
                                       (void (*)(s32, s32)) hsd_803A949C);
                hsd_804D799C = 1;
                OSRestoreInterrupts(intr2);
                if (r < 0) {
                    hsd_804D7988 = r;
                    hsd_804D799C = 0;
                    retryCardClose(&CMD_STATE->file_info);
                    continue;
                }
                return;
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 4:
            if (hsd_804D7988 != 1) {
                memset(CMD_X18, 0, CMD_X20);
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 1:
            if (hsd_804D7988 != 1) {
                s32 hdr_offset;
                s32 rem;
                signed int size;
                r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                      &CMD_STATE->file_info);
                if (r < 0) {
                    hsd_804D7988 = r;
                    continue;
                }
                if (CMD_X10 > 0) {
                    hdr_offset = 0;
                } else {
                    hdr_offset = (CMD_STATE->x24 + 0x30) % CMD_STATE->x8;
                }
                size = CMD_X20;
                if (size > 0 && CMD_X18 != NULL) {
                    memcpy(&CARD_BUF(CMD_STATE)[hdr_offset + 0x20], CMD_X18, size);
                }
                rem = (CMD_STATE->x8 - hdr_offset) - size - 0x20;
                if (rem != 0) {
                    memset(&CARD_BUF(CMD_STATE)[hdr_offset + size + 0x20], 0, rem);
                }
                memset(CARD_BUF(CMD_STATE) + hdr_offset, 0, 0x20);
                CARD_BUF(CMD_STATE)[hdr_offset + 0x10] = (u8) (CMD_X10 >> 8);
                CARD_BUF(CMD_STATE)[hdr_offset + 0x11] = (u8) CMD_X10;
                CARD_BUF(CMD_STATE)[hdr_offset + 0x12] = (u8) CMD_X14;
                fn_803AC3F8(CMD_STATE, &CARD_BUF(CMD_STATE)[hdr_offset + 0x13],
                            CMD_X8);
                hsd_803B2FA0(CARD_BUF(CMD_STATE) + hdr_offset,
                             CMD_STATE->x8 - hdr_offset);
                hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
                intr2 = OSDisableInterrupts();
                r = retryCardWriteAsync(&CMD_STATE->file_info, CARD_BUF(CMD_STATE),
                                        CMD_STATE->x8, CMD_X1C,
                                        (void (*)(s32, s32)) hsd_803A949C);
                hsd_804D799C = 1;
                OSRestoreInterrupts(intr2);
                if (r < 0) {
                    hsd_804D7988 = r;
                    hsd_804D799C = 0;
                    retryCardClose(&CMD_STATE->file_info);
                    continue;
                }
                return;
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 16:
            if (hsd_804D7988 != 1) {
                r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                      &CMD_STATE->file_info);
                if (r < 0) {
                    hsd_804D7988 = r;
                    continue;
                }
                hsd_803B2FA0(CARD_BUF(CMD_STATE), CMD_STATE->x8);
                hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
                intr2 = OSDisableInterrupts();
                r = retryCardWriteAsync(&CMD_STATE->file_info, CARD_BUF(CMD_STATE),
                                        CMD_STATE->x8, CMD_X1C,
                                        (void (*)(s32, s32)) hsd_803A949C);
                hsd_804D799C = 1;
                OSRestoreInterrupts(intr2);
                if (r < 0) {
                    hsd_804D7988 = r;
                    hsd_804D799C = 0;
                    retryCardClose(&CMD_STATE->file_info);
                    continue;
                }
                return;
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 7:
            intr2 = OSDisableInterrupts();
            r = retryCardCreateAsync(CMD_STATE->x4, U32_TO_PTR(char*, CMD_X8), CMD_XC,
                                     &CMD_STATE->file_info,
                                     (void (*)(s32, s32)) hsd_803A949C);
            hsd_804D799C = 1;
            OSRestoreInterrupts(intr2);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                continue;
            }
            return;
        case 8:
            if (hsd_804D7988 != 1) {
                int k;
                r = retryCardGetStatus(CMD_STATE->x4, CMD_STATE->x20,
                                       &CMD_STATE->stat);
                if (r < 0) {
                    hsd_804D7988 = r;
                    continue;
                }
                CMD_STATE->stat.commentAddr = 0;
                CMD_STATE->stat.iconAddr = 0x40;
                CMD_STATE->stat.bannerFormat =
                    (CMD_STATE->stat.bannerFormat & ~3) | CMD_STATE->x3B0;
                k = setupCardIcons(cmd);
                for (; k < 8; k++) {
                    CMD_STATE->stat.iconFormat =
                        CMD_STATE->stat.iconFormat &
                        ~(CARD_STAT_ICON_MASK << (2 * k));
                    CMD_STATE->stat.iconSpeed =
                        CMD_STATE->stat.iconSpeed &
                        ~(CARD_STAT_SPEED_MASK << (2 * k));
                }
                intr2 = OSDisableInterrupts();
                r = retryCardSetStatusAsync(CMD_STATE->x4, CMD_STATE->x20,
                                            &CMD_STATE->stat,
                                            (void (*)(s32, s32)) hsd_803A949C);
                hsd_804D799C = 1;
                OSRestoreInterrupts(intr2);
                if (r < 0) {
                    hsd_804D7988 = r;
                    hsd_804D799C = 0;
                    continue;
                }
                return;
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 9:
            if (hsd_804D7988 != 1) {
                s32 icon_size;
                s32 pos;
                r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                      &CMD_STATE->file_info);
                if (r < 0) {
                    hsd_804D7988 = r;
                    continue;
                }
                switch (CMD_STATE->x3B0) {
                case 2:
                    icon_size = 0x1800;
                    break;
                case 1:
                    icon_size = 0xE00;
                    break;
                default:
                    icon_size = 0;
                    break;
                }
                if (CMD_X8 == 0) {
                    memcpy(CARD_BUF(CMD_STATE), CMD_STATE->x370, 0x40);
                    pos = 0x40;
                    if (icon_size > 0) {
                        memcpy(CARD_BUF(CMD_STATE) + 0x40, U32_TO_PTR(void*, CMD_XC),
                               icon_size);
                        pos = icon_size + 0x40;
                    }
                    memset(CMD_STATE->digest, 0, 0x30);
                    if (CMD_STATE->x24 > CMD_STATE->x8) {
                        memcpy(&CARD_BUF(CMD_STATE)[pos], U32_TO_PTR(void*, CMD_X10),
                               CMD_STATE->x8 - pos);
                        hsd_803B2B20(CARD_BUF(CMD_STATE), CMD_STATE->x8,
                                     &CMD_STATE->digest[CMD_X8 * 0x10]);
                    } else {
                        memcpy(&CARD_BUF(CMD_STATE)[pos], U32_TO_PTR(void*, CMD_X10),
                               CMD_STATE->x24 - pos);
                        hsd_803B2B20(CARD_BUF(CMD_STATE), CMD_STATE->x24,
                                     &CMD_STATE->digest[CMD_X8 * 0x10]);
                        memcpy(&CARD_BUF(CMD_STATE)[CMD_STATE->x24],
                               CMD_STATE->digest, 0x30);
                    }
                } else {
                    u32 remaining = CMD_STATE->x24 - CMD_STATE->x8 * CMD_X8;
                    s32 data_off = (CMD_STATE->x8 * CMD_X8 - 0x40) - icon_size;
                    if (remaining > CMD_STATE->x8) {
                        memcpy(CARD_BUF(CMD_STATE), U32_TO_PTR(u8*, CMD_X10) + data_off,
                               CMD_STATE->x8);
                        hsd_803B2B20(CARD_BUF(CMD_STATE), CMD_STATE->x8,
                                     &CMD_STATE->digest[CMD_X8 * 0x10]);
                    } else {
                        memcpy(CARD_BUF(CMD_STATE), U32_TO_PTR(u8*, CMD_X10) + data_off,
                               remaining);
                        hsd_803B2B20(CARD_BUF(CMD_STATE), remaining,
                                     &CMD_STATE->digest[CMD_X8 * 0x10]);
                        memcpy(&CARD_BUF(CMD_STATE)[remaining], CMD_STATE->digest,
                               0x30);
                    }
                }
                intr2 = OSDisableInterrupts();
                r = retryCardWriteAsync(&CMD_STATE->file_info, CARD_BUF(CMD_STATE),
                                        CMD_STATE->x8, CMD_STATE->x8 * CMD_X8,
                                        (void (*)(s32, s32)) hsd_803A949C);
                hsd_804D799C = 1;
                OSRestoreInterrupts(intr2);
                if (r < 0) {
                    hsd_804D799C = 0;
                    retryCardClose(&CMD_STATE->file_info);
                    hsd_804D7988 = r;
                    continue;
                }
                return;
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 12: {
            s32 retries;
            CARDStat stat;
            s32 i;
            s32 file_no;
            CMD_STATE->x20 = CMD_X8;
            file_no = CMD_STATE->x20;
            chan = CMD_STATE->x4;
            for (i = 0; i < 10; i++) {
                r = CARDGetStatus(chan, file_no, &stat);
                if (r != -1) {
                    break;
                }
            }
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
            } else {
                unpackCardStat(cmd, &stat);
                if (stat.iconAddr != 0x40) {
                    hsd_804D7988 = -0x106;
                    hsd_804D799C = 0;
                } else {
                    CMD_TYPE = 0;
                    hsd_804D7980 = (hsd_804D7980 + 1) % 128;
                }
            }
            continue;
        }
        case 13:
            r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                  &CMD_STATE->file_info);
            if (r < 0) {
                hsd_804D7988 = r;
                continue;
            }
            intr2 = OSDisableInterrupts();
            r = retryCardReadAsync(&CMD_STATE->file_info, CARD_BUF(CMD_STATE),
                                   CMD_STATE->x8, CMD_X1C,
                                   (void (*)(s32, s32)) hsd_803A949C);
            hsd_804D799C = 1;
            OSRestoreInterrupts(intr2);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                retryCardClose(&CMD_STATE->file_info);
                continue;
            }
            return;
        case 14:
            r = fn_803AD16C(CMD_STATE);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
            } else {
                CMD_TYPE = 0;
                hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            }
            continue;
        case 17: {
            s32 blk;

            fn_803AC2D4();
            r = fn_803ACF30(CMD_STATE, 0, 0, 0);
            if (r < 0) {
                fn_803AC2E0();
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                continue;
            }
            for (blk = 0; blk <= CMD_STATE->x460; blk++) {
                r = fn_803AC258(CMD_STATE, blk);
                if (r < 0) {
                    fn_803AC2E0();
                    hsd_804D7988 = r;
                    hsd_804D799C = 0;
                    goto next;
                }
            }
            r = fn_803AC2A4(CMD_STATE);
            if (r < 0) {
                fn_803AC2E0();
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                continue;
            }
            fn_803AC334();
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        }
        default:
            return;
        }
    next:
        continue;
    }
}

s32 fn_803AC168(s32* cmd_buf)
{
    BOOL state;
    s32 mode;
    s32 read_idx;
    s32 old_entry;

    state = OSDisableInterrupts();
    read_idx = hsd_804D7980;
    mode = hsd_804D799C;
    old_entry = (s32) hsd_804D1148[hsd_804D7980][0];
    OSRestoreInterrupts(state);

    if (mode != 2) {
        if (hsd_804D7984 == read_idx) {
            if (mode != 0 || hsd_804D7984 != read_idx || old_entry != 0) {
                return -265;
            }
        }
    }

    {
        s32 idx = hsd_804D7984;
        hsd_804D7984 = (hsd_804D7984 + 1) % 128;
        memcpy((u8*) hsd_804D1148[idx], cmd_buf, sizeof(CardCmd));
    }

    if (mode == 2) {
        hsd_804D799C = 0;
    }
    return 0;
}

s32 fn_803AC258(CardState* card_state, s32 block_idx)
{
    CardCmd buf;
    buf.type = 13;
    buf.state = PTR_TO_U32(card_state);
    buf.x10 = block_idx;
    buf.x14 = 0;
    buf.x18 = 0;
    buf.x20 = 0;
    buf.x1C = fn_803ACBE8(card_state, block_idx);
    return fn_803AC168((s32*) &buf);
}

s32 fn_803AC2A4(CardState* card_state)
{
    CardCmd buf;
    buf.type = 14;
    buf.state = PTR_TO_U32(card_state);
    return fn_803AC168((s32*) &buf);
}

void fn_803AC2D4(void)
{
    hsd_804D7998 = hsd_804D7984;
}

void fn_803AC2E0(void)
{
    s32 saved;

    if (hsd_804D7998 < 0) {
        return;
    }
    saved = hsd_804D7998;
    while (saved != hsd_804D7984) {
        hsd_804D1148[saved][0] = 0;
        saved = (saved + 1) % 128;
    }
    hsd_804D7984 = hsd_804D7998;
}

void fn_803AC334(void)
{
    hsd_804D7998 = -1;
}

int hsd_803AC340(void* header)
{
    u8* data = header;
    int base_size;
    int extra_size;
    int has_small;
    int i;

    switch (data[0]) {
    case 2:
        base_size = 0x1800;
        break;
    case 1:
        base_size = 0xE00;
        break;
    default:
        base_size = 0;
        break;
    }

    has_small = 0;
    extra_size = 0;
    i = 0;
    while (i < 8 && data[i + 0xA] != 0) {
        switch (data[i + 0x2]) {
        case 2:
            extra_size += 0x800;
            break;
        case 1:
            has_small = 1;
            extra_size += 0x400;
            break;
        }
        i++;
    }

    if (has_small != 0) {
        extra_size += 0x200;
    }

    extra_size = base_size + extra_size;
    return extra_size + 0x40;
}

void hsd_803AC3E0(struct CardState* file_desc, int file_idx, int file_size,
                  int file_flags, u8* data)
{
    file_desc->x28[file_idx] = file_flags;
    file_desc->x4C[file_idx] = file_size;
    file_desc->x70[file_idx].ptr = PTR_TO_U32(data);
}

void fn_803AC3F8(void* arg0, u8* data, s32 file_idx)
{
    struct CardState* file_desc = arg0;
    s32 start;
    s32 i;

    if (file_idx + 1 >= 9 || file_desc->x4C[file_idx + 1] == 0) {
        start = file_idx - 2;
    } else {
        start = file_idx - 1;
    }
    if (start < 0) {
        start = 0;
    }

    for (i = 0; i < 3 && i < 9; i++) {
        s32 idx = start + i;
        data[0] = idx;
        data[1] = ((((u32) file_desc->x4C[idx]) >> (0, 0x10)) & 0x3F) |
                  ((file_desc->x28[idx] << 6) & 0xC0);
        data[2] = (u32) file_desc->x4C[idx] >> 8;
        data[3] = file_desc->x4C[idx];
        data += 4;
    }

    for (; i < 3; i++) {
        data[0] = 0;
        data[1] = 0;
        data[2] = 0;
        data[3] = 0;
        data += 4;
    }
}

void hsd_803AC558(struct CardState* file_desc, u8* data)
{
    int i;

    for (i = 0; i < 3; i++) {
        u8 file_idx = *data++;
        u8 byte1 = data[0];
        u32 size = (u32) data[1] | ((byte1 << 8) & 0x3F00);
        u32 final_size;
        u8 flags = (byte1 >> 6) & 3;
        size <<= 8;
        final_size = size | data[2];
        data += 3;

        if (final_size != 0 && file_desc->x4C[file_idx] == 0) {
            file_desc->x28[file_idx] = flags;
            file_desc->x4C[file_idx] = final_size;
        }
    }
}

u32 fn_803AC634(struct CardState* file_desc, s32 file_idx)
{
    if (file_desc->x4C[file_idx] <= 0) {
        return 0;
    }

    if (file_idx == 0) {
        u32 usable;
        u32 sector_size = file_desc->x8;
        s32 remaining;

        remaining = file_desc->x4C[0];
        remaining = remaining - (s32) ((usable = sector_size - 0x20) -
                                       (file_desc->x24 + 48) % sector_size);
        if (remaining <= 0) {
            return 1;
        }
        return (u32) (remaining + sector_size - 0x21) / usable + 1;
    }

    {
        u32 sector_size = file_desc->x8;
        return (u32) (file_desc->x4C[file_idx] + sector_size - 0x21) /
               (sector_size - 0x20);
    }
}

static inline u32 fn_803AC6B8_first_block_count(struct CardState* file_desc)
{
    if (file_desc->x4C[0] <= 0) {
        return 0;
    }

    {
        u32 usable;
        u32 sector_size = file_desc->x8;
        s32 remaining;

        remaining = file_desc->x4C[0];
        remaining = remaining - (s32) ((usable = sector_size - 0x20) -
                                       (file_desc->x24 + 48) % sector_size);
        if (remaining <= 0) {
            return 1;
        }
        return (u32) (remaining + sector_size - 0x21) / usable + 1;
    }
}

s32 fn_803AC6B8(struct CardState* file_desc, s32 file_count)
{
    u32 sector_size;
    s32 remaining;
    u32 usable;
    s32 blocks;
    s32 file_size;
    s32 total;
    s32 i;

    if (file_count >= 9) {
        return 0;
    }
    if (file_count == 0) {
        return 0;
    }

    total = 1;
    if (file_desc->x4C[0] > 0) {
        total += fn_803AC6B8_first_block_count(file_desc) - 1;
    }

    for (i = 1; i < file_count; i++) {
        file_size = file_desc->x4C[i];
        if (file_size <= 0) {
            blocks = 0;
        } else if (i == 0) {
            sector_size = file_desc->x8;
            remaining = file_desc->x4C[0] -
                        (s32) ((usable = sector_size - 0x20) -
                               (file_desc->x24 + 48) % sector_size);
            if (remaining <= 0) {
                blocks = 1;
            } else {
                blocks = (u32) (remaining + sector_size - 0x21) / usable + 1;
            }
        } else {
            sector_size = file_desc->x8;
            blocks =
                (u32) (file_size + sector_size - 0x21) / (sector_size - 0x20);
        }
        total += blocks;
    }

    return total;
}

static inline s32 fn_803AC6B8_blocks_before(struct CardState* file_desc,
                                            s32 file_count)
{
    s32 total;
    s32 i;

    if (file_count >= 9) {
        return 0;
    }
    if (file_count == 0) {
        return 0;
    }

    total = 1;
    if (file_desc->x4C[0] > 0) {
        total = fn_803AC634(file_desc, 0);
    }

    for (i = 1; i < file_count; i++) {
        total += fn_803AC634(file_desc, i);
    }

    return total;
}

static inline s32 fn_803AC7DC_block_count(struct CardState* file_state,
                                          s32 file_idx)
{
    s32 blocks;

    if (file_state->x4C[file_idx] <= 0) {
        blocks = 0;
    } else if (file_idx == 0) {
        u32 usable;
        u32 sector_size = file_state->x8;
        s32 remaining;

        remaining = file_state->x4C[0];
        remaining = remaining - (s32) ((usable = sector_size - 0x20) -
                                       (file_state->x24 + 48) % sector_size);
        if (remaining <= 0) {
            blocks = 1;
        } else {
            blocks = (u32) (remaining + sector_size - 0x21) / usable + 1;
        }
    } else {
        u32 sector_size = file_state->x8;
        blocks = (u32) (file_state->x4C[file_idx] + sector_size - 0x21) /
                 (sector_size - 0x20);
    }
    return blocks;
}

s32 fn_803AC7DC(CardState* state)
{
    s32 total = 0;
    s32 extra = 0;
    s32 i;

    if (state->x4C[0] > 0) {
        total = fn_803AC7DC_block_count(state, 0) - 1;
    }

    for (i = 1; i < 9; i++) {
        s32 size = state->x4C[i];

        if (size <= 0) {
            continue;
        }

        switch (state->x28[i]) {
        case 0:
            total += fn_803AC7DC_block_count(state, i) * 2;
            break;
        case 1:
            total += fn_803AC7DC_block_count(state, i);
            if (extra < fn_803AC7DC_block_count(state, i)) {
                extra = fn_803AC7DC_block_count(state, i);
            }
            break;
        case 2:
            total += fn_803AC7DC_block_count(state, i);
            if (extra < 1) {
                extra = 1;
            }
            break;
        case 3:
            total += fn_803AC7DC_block_count(state, i);
            break;
        }
    }

    return total + extra;
}

static inline s32 fn_803ACB74(s32 seq_a, s32 seq_b)
{
    if (seq_a < 0) {
        return -1;
    }
    if (seq_b < 0) {
        return 1;
    }
    if (seq_a == 0 && seq_b == 0xFF) {
        return 1;
    }
    if (seq_a == 0xFF && seq_b == 0) {
        return -1;
    }
    if (seq_a - seq_b > 0x80) {
        return -1;
    }
    if (seq_a - seq_b < -0x80) {
        return 1;
    }
    return seq_a - seq_b;
}

s32 fn_803ACBE8(CardState* state, s32 block_idx)
{
    u32 size = state->x8;
    u32 temp = state->x24 + size;
    u32 num = temp + 0x2F;

    temp = num / size;
    {
        u32 idx = temp - 1;
        idx = block_idx + idx;
        return size * idx;
    }
}
s32 fn_803ACC0C(CardState* state, s32 block_idx, s32 file_id, s32 seq_num,
                void* expected_data, s32 data_size)
{
    s32 result;
    s32 retries;
    u8* buf;
    u32 sector_size;
    u8* data;
    u32 read_offset;
    s32 offset;
    PAD_STACK(4);

    if (data_size == 0) {
        return 0;
    }

    sector_size = state->x8;
    retries = 0;
    buf = CARD_BUF(state);
    {
        u32 temp = state->x24 + sector_size;
        u32 num = temp + 0x2F;

        temp = num / sector_size;
        {
            u32 idx = temp - 1;
            idx = block_idx + idx;
            read_offset = sector_size * idx;
        }
    }

    for (; retries < 10; retries++) {
        {
            s32 tmp =
                CARDRead(&state->file_info, buf, sector_size, read_offset);
            result = tmp;
        }
        if (result != -1) {
            break;
        }
    }

    if (result < 0) {
        return result;
    }

    if (block_idx == 0) {
        offset = (state->x24 + 48) % state->x8;
    } else {
        offset = 0;
    }

    if (hsd_803B31CC(CARD_BUF(state) + offset, state->x8 - offset) < 0) {
        return 1;
    }

    data = CARD_BUF(state) + offset;
    if (file_id != ((data[0x10] << 8) | data[0x11])) {
        return 1;
    }

    if (seq_num != data[0x12]) {
        return 1;
    }

    if (memcmp(expected_data, data + 0x20, data_size) != 0) {
        return 1;
    }

    return 0;
}

s32 fn_803ACD58(CardState* state, void* icon_data, void* file_data)
{
    s32 icon_size;
    s32 i;
    s32 hdr_plus_icon;

    switch (state->x3B0) {
    case 2:
        icon_size = 0x1800;
        break;
    case 1:
        icon_size = 0xE00;
        break;
    default:
        icon_size = 0;
        break;
    }

    hdr_plus_icon = icon_size + 0x40;

    {
        s32 retries;
        u8* buf;
        s32 offset;
        s32 size;
        for (i = 0; size = state->x8, i < (0x2F + state->x24 + size) / size;
             i++)
        {
            s32 result;

            offset = i * size;
            buf = CARD_BUF(state);
            for (retries = 0; retries < 10; retries++) {
                result = CARDRead(&state->file_info, buf, size, offset);
                if (result != -1) {
                    break;
                }
            }

            if (result < 0) {
                return result;
            }

            if (i == 0) {
                if (memcmp(CARD_BUF(state), state->x370, 0x40) != 0) {
                    return 1;
                }
                if (icon_size > 0) {
                    if (memcmp(CARD_BUF(state) + 0x40, icon_data, icon_size) != 0) {
                        return 1;
                    }
                }
                {
                    u32 total_size = state->x24;
                    u32 ss = state->x8;
                    if (total_size > ss) {
                        if (memcmp(CARD_BUF(state) + hdr_plus_icon, file_data,
                                   ss - hdr_plus_icon) != 0)
                        {
                            return 1;
                        }
                    } else {
                        if (memcmp(CARD_BUF(state) + hdr_plus_icon, file_data,
                                   total_size - hdr_plus_icon) != 0)
                        {
                            return 1;
                        }
                    }
                }
            } else {
                u32 ss = state->x8;
                u32 remaining = state->x24 - i * ss;
                s32 data_offset = i * ss - 0x40 - icon_size;

                if (remaining > ss) {
                    if (memcmp(CARD_BUF(state), (u8*) file_data + data_offset, ss) !=
                        0)
                    {
                        return 1;
                    }
                } else {
                    if (memcmp(CARD_BUF(state), (u8*) file_data + data_offset,
                               remaining) != 0)
                    {
                        return 1;
                    }
                }
            }
        }
    }

    return 0;
}

s32 fn_803ACF30(CardState* state, s32 file_id, s32 seq_num, s32 version)
{
    CardCmd buf;
    s32 ret;
    u32 i;
    PAD_STACK(4);

    for (i = 0; i < (0x2F + state->x24 + state->x8) / state->x8; i++) {
        buf.type = 11;
        buf.state = PTR_TO_U32(state);
        buf.x8 = i;
        buf.xC = file_id;
        buf.x10 = seq_num;
        buf.x14 = version;
        ret = fn_803AC168((s32*) &buf);
        if (ret < 0) {
            return ret;
        }
    }
    return 0;
}

typedef u8* CardSectorPtr;

static inline u8* fn_803ACFC0_header(CardState* state, s32 hdr_offset)
{
    return CARD_BUF(state) + hdr_offset;
}

static inline u8* fn_803ACFC0_checksum_start(s32 hdr_offset, CardState* state)
{
    return hdr_offset + CARD_BUF(state);
}

s32 fn_803ACFC0(CardState* state, s32 block_idx, s32 file_id, s32 seq_num,
                void* payload, s32 payload_size, s32 version)
{
    u32 sector_size;
    s32 retries;
    u8* buf;
    s32 offset;
    s32 result;
    s32 hdr_offset;
    PAD_STACK(8);

    sector_size = state->x8;
    {
        u32 hdr_start = state->x24 + 48;
        offset = sector_size *
                 (block_idx + (hdr_start + sector_size - 1) / sector_size - 1);

        if (block_idx == 0) {
            if (file_id != 0) {
                return -257;
            }

            buf = CARD_BUF(state);
            hdr_offset = hdr_start % sector_size;
            for (retries = 0; retries < 10; retries++) {
                result = CARDRead(&state->file_info, buf, sector_size, offset);
                if (result != -1) {
                    break;
                }
            }

            if (result < 0) {
                return result;
            }
        } else {
            hdr_offset = 0;
        }
    }

    if (payload_size > 0) {
        memcpy(&fn_803ACFC0_header(state, hdr_offset)[0x20], payload,
               payload_size);
    }

    {
        s32 remaining = (state->x8 - hdr_offset) - payload_size - 0x20;
        if (remaining != 0) {
            s32 off = payload_size + 0x20;
            memset(CARD_BUF(state) + hdr_offset + off, 0, remaining);
        }
    }

    memset(CARD_BUF(state) + hdr_offset, 0, 0x20);

    (CARD_BUF(state) + hdr_offset)[0x10] = (u8) (file_id >> 8);
    (CARD_BUF(state) + hdr_offset)[0x11] = (u8) file_id;
    (CARD_BUF(state) + hdr_offset)[0x12] = (u8) seq_num;

    fn_803AC3F8(state, fn_803ACFC0_header(state, hdr_offset) + 0x13, version);
    hsd_803B2FA0(fn_803ACFC0_checksum_start(hdr_offset, state),
                 state->x8 - hdr_offset);

    {
        s32 write_retries;
        u8* write_buf;
        u32 write_size;
        write_size = state->x8;
        write_retries = 0;
        write_buf = CARD_BUF(state);
        for (; write_retries < 10; write_retries++) {
            result =
                CARDWrite(&state->file_info, write_buf, write_size, offset);
            if (result != -1) {
                break;
            }
        }
    }

    return result;
}

static inline int fn_803AD16C_file_type_nonzero(CardState* state, s32 file_id)
{
    return state->x28[file_id] != 0;
}

static inline s32 fn_803AD16C_total_blocks(CardState* state)
{
    return state->x460;
}

static inline s32 fn_803AD16C_logical_index(s32 blocks_before, s32 i)
{
    return blocks_before + i;
}

static inline s32 fn_803AD16C_file_size(CardState* state, s32 file_id)
{
    return state->x4C[file_id];
}

static inline s32 fn_803AD16C_seq_at(s32* seq, s32 phys)
{
    return seq[phys];
}

static inline int fn_803AD16C_nonnegative(s32 logical)
{
    return logical >= 0;
}

static inline s32 fn_803AD16C_queue_cmd(s32* cmd)
{
    return fn_803AC168(cmd);
}

static inline int fn_803AD16C_same(s32 lhs, s32 rhs)
{
    return lhs == rhs;
}

static inline void fn_803AD16C_own(void* value)
{
    (void) value;
}

static inline s32 fn_803AD16C_queue_clear(CardState* state, s32 block, s32 pad,
                                          s32 file_id)
{
    s32 cmd[9];

    cmd[0] = 1;
    cmd[1] = PTR_TO_U32(state);
    cmd[3] = block;
    cmd[4] = 0xFFFF;
    cmd[5] = 0;
    cmd[6] = 0;
    cmd[8] = 0;
    cmd[7] = pad;
    cmd[2] = file_id;
    return fn_803AD16C_queue_cmd(cmd);
}

static inline s32 fn_803AD16C_queue_read(CardState* state, s32 block)
{
    s32 cmd[8];
    u32 size = state->x8;
    u32 temp = state->x24 + size;
    u32 num = temp + 0x2F;
    u32 idx;
    s32 pad;

    temp = num / size;
    idx = temp - 1;
    idx = block + idx;
    pad = size * idx;

    cmd[0] = 0xF;
    cmd[1] = PTR_TO_U32(state);
    cmd[3] = block;
    cmd[7] = pad;
    return fn_803AC168(cmd);
}

static inline s32 fn_803AD16C_queue_write(CardState* state, s32 block,
                                          s32 logical, s32 target_seq)
{
    s32 cmd[8];
    u32 size = state->x8;
    u32 temp = state->x24 + size;
    u32 num = temp + 0x2F;
    u32 idx;
    s32 pad;

    temp = num / size;
    idx = temp - 1;
    idx = block + idx;
    pad = size * idx;

    cmd[0] = 0x10;
    cmd[1] = PTR_TO_U32(state);
    cmd[3] = block;
    cmd[4] = logical;
    cmd[5] = target_seq;
    cmd[7] = pad;
    return fn_803AC168(cmd);
}

static inline s32 fn_803AD16C_queue_write_last(CardState* state, s32 block,
                                               s32 logical, s32 target_seq)
{
    s32 tail[2];
    s32 cmd[8];
    u32 size = state->x8;
    u32 temp = state->x24 + size;
    u32 num = temp + 0x2F;
    u32 idx;
    s32 pad;

    temp = num / size;
    idx = temp - 1;
    idx = block + idx;
    pad = size * idx;

    cmd[0] = 0x10;
    cmd[1] = PTR_TO_U32(state);
    cmd[3] = block;
    cmd[4] = logical;
    cmd[5] = target_seq;
    cmd[7] = pad;
    fn_803AD16C_own(tail);
    return fn_803AC168(cmd);
}

s32 fn_803AD16C(CardState* state)
{
    s32 work[64];
    s32 seq[64];
    s32 newmap[64];
    s32 pad;
    s32 filemap[64];
    s32 chosen[64];
    s32 result;
    s32 file_id;
    s32 pass;
    s32 i;
    s32 phys;
    s32 ofs;
    s32 blocks_before;
    s32 file_blocks;
    PAD_STACK(56);

    result = 0;
    for (i = 0; i <= state->x460; i++) {
        newmap[i] = -1;
        work[i] = state->x170[i];
        seq[i] = state->x270[i];
    }

    if (state->x460 != fn_803AC7DC(state)) {
        return -257;
    }

    for (file_id = 0; file_id < 9; file_id++) {
        file_blocks = fn_803AC634(state, file_id);
        if (file_blocks <= 0) {
            continue;
        }

        blocks_before = fn_803AC6B8_blocks_before(state, file_id);

        for (i = 0; i < file_blocks; i++) {
            filemap[i] = -1;
            chosen[i] = -1;
        }

        for (pass = 0; pass <= state->x460; pass++) {
            s32 cur_seq = -1;

            for (phys = 0; phys <= fn_803AD16C_total_blocks(state); phys++) {
                if (work[phys] >= 0) {
                    s32 logical = work[phys] - blocks_before;
                    if (logical >= 0 && logical < file_blocks) {
                        if (cur_seq == -1 ||
                            fn_803ACB74(cur_seq,
                                        fn_803AD16C_seq_at(seq, phys)) > 0)
                        {
                            cur_seq = seq[phys];
                        }
                    }
                }
            }
            if (cur_seq < 0) {
                break;
            }

            for (phys = 0; phys <= state->x460; phys++) {
                s32 logical = work[phys] - blocks_before;
                if (fn_803AD16C_nonnegative(logical) &&
                    logical < file_blocks && cur_seq == seq[phys])
                {
                    filemap[logical] = phys;
                }
            }

            {
                for (i = 0; i < file_blocks; i++) {
                    if (filemap[i] < 0 || cur_seq != seq[filemap[i]]) {
                        break;
                    }
                }
                if (i == file_blocks) {
                    for (i = 0; i < file_blocks; i++) {
                        chosen[i] = filemap[i];
                    }
                }
            }

            for (i = 0; i < file_blocks; i++) {
                if (filemap[i] >= 0) {
                    work[filemap[i]] = -0x7FFF;
                }
            }
        }

        if (chosen[0] >= 0) {
            for (i = 0; i < file_blocks; i++) {
                newmap[blocks_before + i] = chosen[i];
            }
        } else {
            for (i = 0; i < file_blocks; i++) {
                {
                    s32 tmp = filemap[i];
                    if (tmp < 0) {
                        result = -0x103;
                    } else {
                        newmap[blocks_before + i] = filemap[i];
                        if (result != -0x103) {
                            result = -0x104;
                        }
                    }
                }
            }
        }
    }

    for (file_id = 0; file_id < 9; file_id++) {
        if (fn_803AD16C_file_size(state, file_id) <= 0 ||
            (state->x28[file_id] != 1 && state->x28[file_id] != 2))
        {
            continue;
        }

        file_blocks = fn_803AC634(state, file_id);
        blocks_before = fn_803AC6B8_blocks_before(state, file_id);

        for (i = 0; i < file_blocks; i++) {
            s32 logical = fn_803AD16C_logical_index(blocks_before, i);
            if (newmap[logical] >= 0) {
                for (phys = 1; phys <= state->x460; phys++) {
                    if (phys != newmap[logical] &&
                        state->x170[phys] == logical)
                    {
                        state->x170[phys] = -logical;
                    }
                }
            }
        }
    }

    for (file_id = 0; file_id < 9; file_id++) {
        if (state->x4C[file_id] <= 0 ||
            fn_803AD16C_file_type_nonzero(state, file_id))
        {
            continue;
        }

        file_blocks = fn_803AC634(state, file_id);
        blocks_before = fn_803AC6B8_blocks_before(state, file_id);

        for (i = 0; i < file_blocks; i++) {
            s32 logical = blocks_before + i;
            s32 src = newmap[logical];
            s32 dup;
            s32 target_seq;
            s32 ret;

            if (fn_803AD16C_same(file_id, file_id)) {
                (void) logical;
            }

            if (newmap[logical] < 0) {
                continue;
            }

            target_seq = state->x270[src];
            dup = -1;
            for (phys = 1; phys <= state->x460; phys++) {
                if (phys != newmap[logical] && state->x170[phys] == logical) {
                    if (dup < 0) {
                        dup = phys;
                    } else {
                        pad = fn_803ACBE8(state, phys);
                        if (phys == 0) {
                            ret = -0x101;
                        } else {
                            ret = fn_803AD16C_queue_clear(state, phys, pad,
                                                          file_id);
                        }
                        if (ret < 0 && result == 0) {
                            result = -0x10B;
                        }
                        state->x170[phys] = -0x7FFF;
                        state->x270[phys] = 0;
                    }
                }
            }

            if (dup == -1) {
                for (phys = state->x460; phys > 0; phys--) {
                    if (state->x170[phys] < 0) {
                        dup = phys;
                        break;
                    }
                }
                if (dup >= 0) {
                    if (newmap[logical] == 0 || dup == 0) {
                        ret = -0x101;
                    } else {
                        ret = fn_803AD16C_queue_read(state, newmap[logical]);
                        do {
                            if (ret < 0) {
                                if (ret == -0x101) {
                                    continue;
                                }
                                continue;
                            }
                            {
                                ret = fn_803AD16C_queue_write(
                                    state, dup, logical, target_seq);
                            }
                        } while (0);
                    }
                    if (ret < 0 && (result == 0 || result == -0x10B)) {
                        result = -0x102;
                    }
                }
            } else if (state->x270[dup] != target_seq) {
                if (newmap[logical] == 0 || dup == 0) {
                    ret = -0x101;
                } else {
                    {
                        s32 cmd_result =
                            fn_803AD16C_queue_read(state, newmap[logical]);
                        ret = cmd_result;
                    }
                    do {
                        if (ret < 0) {
                            if (ret == -0x101) {
                                continue;
                            }
                            continue;
                        }
                        {
                            ret = fn_803AD16C_queue_write_last(
                                state, dup, logical, target_seq);
                        }
                    } while (0);
                }
                if (ret < 0 && (result == 0 || result == -0x10B)) {
                    result = -0x102;
                }
            }
        }
    }

    return result;
}

s32 fn_803ADE4C(s32 card_state, s32 channel, s32 callback)
{
    CardCmd buf1;
    CardCmd buf2;
    s32 result;
    s32 saved;
    s32 snap;
    s32 saved1;
    s32 snap1;
    CardBufEntry* entries = (CardBufEntry*) hsd_804D1138;
    PAD_STACK(16);

    hsd_804D7998 = hsd_804D7984;
    buf1.type = 12;
    buf1.state = (u32) card_state;
    buf1.x8 = channel;
    result = fn_803AC168((s32*) &buf1);
    if (result < 0) {
        snap1 = hsd_804D7998;
        if (snap1 >= 0) {
            saved1 = snap1;
            while (saved1 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved1].x10 = 0;
                saved1 = (saved1 + 1) % 128;
            }
            hsd_804D7984 = snap1;
        }
        return result;
    }

    buf2.type = 17;
    buf2.state = (u32) card_state;
    result = fn_803AC168((s32*) &buf2);
    if (result < 0) {
        snap = hsd_804D7998;
        if (snap >= 0) {
            saved = snap;
            while (saved != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved].x10 = 0;
                saved = (saved + 1) % 128;
            }
            hsd_804D7984 = snap;
        }
        return result;
    }

    entries[0].x0 = 5;
    entries[0].x4 = card_state;
    entries[0].x8 = callback;
    entries[0].xC = 0;
    hsd_804D7998 = -1;
    return 0;
}

static inline s32 queueCardCommand2First(CardState* state, s32 block,
                                         void* data, s32 length, s32 offset)
{
    CardCmd command;

    command.type = 2;
    command.state = PTR_TO_U32(state);
    command.x10 = block;
    command.x14 = 0;
    command.x18 = PTR_TO_U32(data);
    command.x20 = length;
    command.x1C = offset;
    return fn_803AC168((s32*) &command);
}

static inline s32 queueCardCommand2Final(CardState* state, s32 block,
                                         void* data, s32 length, s32 offset)
{
#ifdef MUST_MATCH
    CardCmdStorage storage;

    /// @todo MWCC colors @c storage at 0x60(r1), but the original command is
    /// at 0x44(r1). A normal local @c CardCmd matches every instruction except
    /// these stack displacements. This still addresses before the C object;
    /// reproduce that stack coloring and remove the matching-only adjustment.
    MATCH_CARD_CMD_FIELD(storage, type, 0x1C) = 2;
    MATCH_CARD_CMD_FIELD(storage, state, 0x1C) = (s32) state;
    MATCH_CARD_CMD_FIELD(storage, x10, 0x1C) = block;
    MATCH_CARD_CMD_FIELD(storage, x14, 0x1C) = 0;
    MATCH_CARD_CMD_FIELD(storage, x18, 0x1C) = (s32) data;
    MATCH_CARD_CMD_FIELD(storage, x20, 0x1C) = length;
    MATCH_CARD_CMD_FIELD(storage, x1C, 0x1C) = offset;
    return fn_803AC168((s32*) ((u8*) storage.words - 0x1C));
#else
    CardCmd command;

    command.type = 2;
    command.state = PTR_TO_U32(state);
    command.x10 = block;
    command.x14 = 0;
    command.x18 = PTR_TO_U32(data);
    command.x20 = length;
    command.x1C = offset;
    return fn_803AC168((s32*) &command);
#endif
}

static inline s32 calculateDataBlockSize(CardState* state, s32 file_idx,
                                         s32 block_idx)
{
    if (file_idx == 0 && block_idx == 0) {
        return (state->x8 - 0x20) - ((state->x24 + 0x30) % state->x8);
    }
    return state->x8 - 0x20;
}

static inline s32 calculateFileBlockCount(CardState* state, s32 file_idx)
{
    s32 file_size = state->x4C[file_idx];

    if (file_size <= 0) {
        return 0;
    } else if (file_idx == 0) {
        u32 sector_size = state->x8;
        u32 usable;
        s32 rem;

        rem = state->x4C[0];
        usable = sector_size - 0x20;
        rem = rem - (s32) (usable - (state->x24 + 0x30) % sector_size);
        if (rem <= 0) {
            return 1;
        } else {
            return (u32) (rem + sector_size - 0x21) / (sector_size - 0x20) + 1;
        }
    } else {
        u32 sector_size = state->x8;
        return (u32) (file_size + sector_size - 0x21) / (sector_size - 0x20);
    }
}

static inline s32 retryCardRead(CARDFileInfo* info, s32 retries, void* buffer,
                                s32 length, s32 offset)
{
    s32 result;

    for (; retries < 10; retries++) {
        result = CARDRead(info, buffer, length, offset);
        if (result != -1) {
            break;
        }
    }
    return result;
}

static inline void cancelQueuedCardCommands(CardBufEntry* entries)
{
    s32 snap = hsd_804D7998;

    if (snap >= 0) {
        s32 saved = snap;
        s32 zero;

        while (saved != hsd_804D7984) {
            CardBufEntry* queued = &entries[saved];
            saved = (saved + 1) % 128;
            queued->x10 = zero = 0;
        }
        hsd_804D7984 = snap;
    }
}

static inline s32 queueClearDataBlock(CardState* state, const u8* dst,
                                      s32 size)
{
#ifdef MUST_MATCH
    CardCmdStorage storage;

    /// @todo MWCC colors @c storage at 0xC4(r1), but the original command is
    /// at 0x68(r1). A normal local @c CardCmd matches every instruction except
    /// these stack displacements. This still addresses before the C object;
    /// reproduce that stack coloring and remove the matching-only adjustment.
    MATCH_CARD_CMD_FIELD(storage, type, 0x5C) = 4;
    MATCH_CARD_CMD_FIELD(storage, state, 0x5C) = (s32) state;
    MATCH_CARD_CMD_FIELD(storage, x10, 0x5C) = 0;
    MATCH_CARD_CMD_FIELD(storage, x14, 0x5C) = 0;
    MATCH_CARD_CMD_FIELD(storage, x18, 0x5C) = (s32) dst;
    MATCH_CARD_CMD_FIELD(storage, x20, 0x5C) = size;
    MATCH_CARD_CMD_FIELD(storage, x1C, 0x5C) = 0;
    MATCH_CARD_CMD_FIELD(storage, x8, 0x5C) = 0;
    return fn_803AC168((s32*) ((u8*) storage.words - 0x5C));
#else
    CardCmd command;

    command.type = 4;
    command.state = PTR_TO_U32(state);
    command.x10 = 0;
    command.x14 = 0;
    command.x18 = PTR_TO_U32(dst);
    command.x20 = size;
    command.x1C = 0;
    command.x8 = 0;
    return fn_803AC168((s32*) &command);
#endif
}

static inline s32 queueDataBlockFirst(CardState* state, s32 block_idx, u8* dst,
                                      s32 size)
{
    return queueCardCommand2First(state, block_idx, dst, size,
                                  fn_803ACBE8(state, block_idx));
}

static inline s32 queueDataBlockFinal(CardState* state, s32 block_idx, u8* dst,
                                      s32 size)
{
    return queueCardCommand2Final(state, block_idx, dst, size,
                                  fn_803ACBE8(state, block_idx));
}

static inline s32 cardDataBlockOffset(CardState* state, u32 sector_size,
                                      s32 data_block)
{
    u32 temp = state->x24 + sector_size;
    u32 num = temp + 0x2F;
    u32 idx;

    temp = num / sector_size;
    idx = temp - 1;
    idx = data_block + idx;
    return sector_size * idx;
}

static inline s32 readCardDataBlockFirst(CardState* state, u32 sector_size,
                                         s32 data_block, u8* dst, s32 length)
{
    s32 offset;
    u8* buf;
    s32 retries;
    s32 read_ofs;
    int result;

    offset = cardDataBlockOffset(state, sector_size, data_block);
    buf = CARD_BUF(state);
    retries = 0;
    result =
        retryCardRead(&state->file_info, retries, buf, sector_size, offset);
    if (result < 0) {
        return result;
    }

    read_ofs = data_block == 0 ? (state->x24 + 0x30) % state->x8 : 0;
    if (hsd_803B31CC(CARD_BUF(state) + read_ofs, state->x8 - read_ofs) < 0) {
        return -0x105;
    }
    if (length != 0 && dst != NULL) {
        memcpy(dst, CARD_BUF(state) + (read_ofs + 0x20), length);
    }
    return 0;
}

static inline s32 readCardDataBlockFinal(CardState* state, u32 sector_size,
                                         s32 data_block, u8* dst, s32 length)
{
    u32 read_offset;
    u8* buf;
    s32 retries;
    s32 read_ofs;
    s32 result;

    retries = 0;
    buf = CARD_BUF(state);
    {
        u32 temp = state->x24 + sector_size;
        u32 num = temp + 0x2F;
        temp = num / sector_size;
        {
            u32 idx = temp - 1;
            idx = data_block + idx;
            read_offset = sector_size * idx;
        }
    }
    for (; retries < 10; retries++) {
        result = CARDRead(&state->file_info, buf, sector_size, read_offset);
        if (result != -1) {
            break;
        }
    }
    if (result < 0) {
        return result;
    }

    read_ofs = data_block == 0 ? (state->x24 + 0x30) % state->x8 : 0;
    if (hsd_803B31CC(CARD_BUF(state) + read_ofs, state->x8 - read_ofs) < 0) {
        return -0x105;
    }
    if (length != 0 && dst != NULL) {
        memcpy(dst, CARD_BUF(state) + (read_ofs + 0x20), length);
    }
    return 0;
}

static inline s32 loadCardDataBlock(s32 data_block)
{
    return data_block;
}

#ifdef __MWERKS__
#pragma opt_loop_invariants off
#endif
s32 fn_803ADF90(struct CardState* arg0, s32 arg1, u8* arg2, s32 arg3,
                void (*arg4)(s32, s32))
{
    CardBufEntry* entries = (CardBufEntry*) hsd_804D1138;
    u8* dst;
    s32 remaining;
    s32 block_map[64];
    int blocks_before;
    int i;
    int logical;
    int scan_idx;
    int result;
    int file_blocks;
    s32 chunk;
    s32 total_blocks;
    s32 file_size;
    s32 callback_seq;

    PAD_STACK(8);

    callback_seq = 0;
    if (arg3 == 0) {
        BOOL intr = OSDisableInterrupts();
        s32 busy;
        if (hsd_804D799C == 1) {
            busy = 1;
        } else {
            busy = 0;
        }
        OSRestoreInterrupts(intr);
        if (busy) {
            return -264;
        }
    }

    blocks_before = fn_803AC6B8_blocks_before(arg0, arg1);

    file_size = arg0->x4C[arg1];
    file_blocks = calculateFileBlockCount(arg0, arg1);

    total_blocks = fn_803AC7DC(arg0);
    for (i = 0; i < file_blocks; i++) {
        block_map[i] = -1;
    }

    chunk = -1;
    for (scan_idx = 1; scan_idx <= total_blocks; scan_idx++) {
        logical = arg0->x170[scan_idx] - blocks_before;
        if (logical >= 0 && logical < file_blocks) {
            if (chunk < 0) {
                chunk = arg0->x270[scan_idx];
            } else {
                if (chunk != arg0->x270[scan_idx]) {
                    callback_seq = -260;
                }
                if (fn_803ACB74(chunk, arg0->x270[scan_idx]) > 0) {
                    chunk = arg0->x270[scan_idx];
                }
            }
            block_map[logical] = scan_idx;
        }
    }

    if (arg1 == 0 && arg0->x170[0] == 0) {
        block_map[0] = 0;
        if (chunk >= 0 && chunk != arg0->x270[0]) {
            callback_seq = -260;
        }
    }

    for (i = 0; i < file_blocks; i++) {
        if (block_map[i] < 0) {
            callback_seq = -259;
            break;
        }
    }

    if (callback_seq != 0) {
        return callback_seq;
    }

    if (arg3 != 0) {
        hsd_804D7998 = hsd_804D7984;
    } else {
        result = retryCardFastOpen(arg0->x4, arg0->x20, &arg0->file_info);
        if (result < 0) {
            return result;
        }
    }

    dst = arg2;
    remaining = file_size;

    for (i = 0; i < file_blocks && remaining > 0; i++) {
        chunk = calculateDataBlockSize(arg0, arg1, i);

        if (remaining > chunk) {
            u32 sector_size;
            s32 data_block;
            data_block = loadCardDataBlock(block_map[i]);
            if (data_block >= 0) {
                if (arg3 != 0) {
                    result = blocks_before =
                        queueDataBlockFirst(arg0, data_block, dst, chunk);
                    if (result < 0) {
                        cancelQueuedCardCommands(entries);
                        return result;
                    }
                } else {
                    sector_size = arg0->x8;
                    result = readCardDataBlockFirst(arg0, sector_size,
                                                    data_block, dst, chunk);
                    if (result < 0) {
                        if (result == -0x105) {
                            callback_seq = -259;
                        } else {
                            callback_seq = -259;
                        }
                    }
                }
            } else if (arg3 != 0) {
                if (chunk == 0) {
                    result = 0;
                } else {
                    result = queueClearDataBlock(arg0, dst, chunk);
                }
                if (result < 0) {
                    cancelQueuedCardCommands(entries);
                    return result;
                }
            } else {
                memset(dst, 0, chunk);
            }

            remaining -= chunk;
            dst += chunk;
        } else {
            u32 sector_size;
            s32 data_block = loadCardDataBlock(block_map[i]);
            if (data_block >= 0) {
                if (arg3 != 0) {
                    result =
                        queueDataBlockFinal(arg0, data_block, dst, remaining);
                    if (result < 0) {
                        cancelQueuedCardCommands(entries);
                        return result;
                    }
                } else {
                    sector_size = arg0->x8;
                    result = readCardDataBlockFinal(
                        arg0, sector_size, data_block, dst, remaining);
                    if (result < 0) {
                        if (result == -0x105) {
                            callback_seq = -259;
                        } else {
                            callback_seq = -259;
                        }
                    }
                }
            }
            remaining = 0;
        }
    }

    if (arg3 == 0) {
        result = retryCardClose(&arg0->file_info);
        if (result < 0) {
            return -267;
        }
    } else {
        entries[0].x0 = 1;
        entries[0].x4 = PTR_TO_U32(arg0);
        entries[0].x8 = PTR_TO_U32(arg4);
        entries[0].xC = arg1;
        hsd_804D7998 = -1;
    }

    return callback_seq;
}
#ifdef __MWERKS__
#pragma opt_loop_invariants on
#endif

static inline void fn_803AE7F8_rewind(CardBufEntry* entries)
{
    s32 snap = hsd_804D7998;

    if (snap >= 0) {
        s32 saved = snap;
        s32 zero;

        while (saved != hsd_804D7984) {
            CardBufEntry* queued = &entries[saved];
            saved = (saved + 1) % 128;
            queued->x10 = zero = 0;
        }
        hsd_804D7984 = snap;
    }
}

static inline void fn_803AE7F8_close(CardState* state)
{
    s32 retries;

    for (retries = 0; retries < 10; retries++) {
        if (CARDClose(&state->file_info) != -1) {
            break;
        }
    }
}

s32 fn_803AE7F8(struct CardState* arg0, s32 arg1, s32 arg2, s32 arg3, s32 arg4)
{
    CardState* state = arg0;
    CardBufEntry* entries = (CardBufEntry*) hsd_804D1138;
    s32 block_map[3][64];
    UNUSED u8 pad_block_map[32];
    CardCmd cmd_done;
    s32* block_map_ptr;
    s32 blocks_before;
    s32 current_seq;
    s32 file_blocks;
    s32 file_size;
    s32 free_count;
    s32 i;
    s32 logical;
    CardState* row;
    s32 repair_result;
    s32 result;
    s32 total_blocks;
    s32 verify_failed;
    u8* data;
    PAD_STACK(24);

    repair_result = 0;
    verify_failed = 0;
    if (arg3 == 0) {
        BOOL intr = OSDisableInterrupts();
        s32 busy;
        if (hsd_804D799C == 1) {
            busy = 1;
        } else {
            busy = 0;
        }
        OSRestoreInterrupts(intr);
        if (busy) {
            return -264;
        }
    }

    blocks_before = fn_803AC6B8_blocks_before(arg0, arg1);

    file_size = state->x4C[arg1];
    file_blocks = fn_803AC634(arg0, arg1);
    total_blocks = fn_803AC7DC(arg0);

    for (i = 0; i < file_blocks; i++) {
        block_map[0][i] = -1;
        block_map[1][i] = -1;
    }

    free_count = 0;
    current_seq = -1;

    for (i = 1, row = (CardState*) ((s32*) state + 1); i <= total_blocks;
         row = (CardState*) ((s32*) row + 1), i++)
    {
        s32 file_idx = row->x170[0];

        if (file_idx < 0) {
            block_map[2][free_count++] = i;
            continue;
        }

        logical = file_idx - blocks_before;
        if (0 <= logical && logical < file_blocks) {
            if (current_seq == -1 ||
                fn_803ACB74(current_seq, row->x270[0]) < 0)
            {
                current_seq = row->x270[0];
            }
            if (block_map[0][logical] < 0) {
                block_map[0][logical] = i;
            } else {
                block_map[1][logical] = i;
            }
        }
    }

    block_map_ptr = block_map[0];
    for (i = 0; i < file_blocks; i++) {
        if (block_map_ptr[i] < 0 ||
            state->x270[block_map_ptr[i]] != current_seq)
        {
            verify_failed = 1;
            break;
        }
        if (block_map_ptr[64 + i] < 0 ||
            state->x270[block_map_ptr[64 + i]] != current_seq)
        {
            verify_failed = 1;
            break;
        }
    }

    for (i = 0; i < file_blocks; i++) {
        if (block_map_ptr[i] < 0) {
            if (free_count > 0) {
                block_map_ptr[i] = block_map[2][--free_count];
            } else {
                return -257;
            }
        }
        if (block_map_ptr[64 + i] < 0) {
            if (free_count > 0) {
                block_map_ptr[64 + i] = block_map[2][--free_count];
            } else {
                repair_result = -258;
            }
        }
    }

    if (arg3 != 0) {
        hsd_804D7998 = hsd_804D7984;
    } else {
        s32 fd;
        s32 ofs;
        s32 open_result;
        s32 retries;

        ofs = state->x20;
        fd = state->x4;
        for (retries = 0; retries < 10; retries++) {
            open_result = CARDFastOpen(fd, ofs, &state->file_info);
            if (open_result != -1) {
                break;
            }
        }

        if (open_result < 0) {
            return open_result;
        }
    }

    if (verify_failed == 0) {
        s32* map;
        s32 pass;

        pass = 0;
        map = block_map_ptr;
        for (; pass < 2; pass++, map += 64) {
            s32 remaining = file_size;

            data = U32_TO_PTR(u8*, arg2);
            for (i = 0; i < file_blocks && remaining > 0; i++) {
                s32 phys;
                s32 chunk = state->x8;

                chunk -= 0x20;
                if ((u32) remaining > (u32) chunk) {
                    phys = map[i];
                    if (phys >= 0) {
                        if (arg3 != 0) {
                            s32 cmd_result;
                            if (chunk == 0) {
                                cmd_result = 0;
                            } else {
                                s32 ofs = fn_803ACBE8(arg0, phys);
                                CardCmd cmd;

                                cmd.type = 5;
                                cmd.state = PTR_TO_U32(arg0);
                                cmd.x10 = blocks_before + i;
                                cmd.x14 = current_seq;
                                cmd.x18 = PTR_TO_U32(data);
                                cmd.x20 = chunk;
                                cmd.x1C = ofs;
                                cmd_result = fn_803AC168((s32*) &cmd);
                            }
                            if (cmd_result < 0) {
                                fn_803AE7F8_rewind(entries);
                                return cmd_result;
                            }
                        } else {
                            s32 verify_result =
                                fn_803ACC0C(arg0, phys, blocks_before + i,
                                            current_seq, data, chunk);

                            result = verify_result;
                            if (result < 0) {
                                fn_803AE7F8_close(state);
                                return result;
                            }
                            if (result > 0) {
                                goto after_verify;
                            }
                        }
                    }

                    remaining -= state->x8 - 0x20;
                    data += state->x8 - 0x20;
                } else {
                    phys = map[i];
                    if (phys >= 0) {
                        if (arg3 != 0) {
                            s32 cmd_result;
                            if (remaining == 0) {
                                cmd_result = 0;
                            } else {
                                s32 ofs = fn_803ACBE8(arg0, phys);
                                CardCmd cmd;

                                cmd.type = 5;
                                cmd.state = PTR_TO_U32(arg0);
                                cmd.x10 = blocks_before + i;
                                cmd.x14 = current_seq;
                                cmd.x18 = PTR_TO_U32(data);
                                cmd.x20 = remaining;
                                cmd.x1C = ofs;
                                cmd_result = fn_803AC168((s32*) &cmd);
                            }
                            if (cmd_result < 0) {
                                fn_803AE7F8_rewind(entries);
                                return cmd_result;
                            }
                        } else {
                            result = fn_803ACC0C(arg0, phys, blocks_before + i,
                                                 current_seq, data, remaining);
                            if (result < 0) {
                                fn_803AE7F8_close(state);
                                return result;
                            }
                            if (result > 0) {
                                goto after_verify;
                            }
                        }
                    }

                    remaining = 0;
                }
            }
        }

        if (arg3 != 0) {
            s32 cmd_result;

            cmd_done.type = 6;
            cmd_done.state = PTR_TO_U32(arg0);
            cmd_result = fn_803AC168((s32*) &cmd_done);
            if (cmd_result < 0) {
                fn_803AE7F8_rewind(entries);
                return cmd_result;
            }
        } else {
            s32 retries;
            for (retries = 0; retries < 10; retries++) {
                result = CARDClose(&state->file_info);
                if (result != -1) {
                    break;
                }
            }

            if (result < 0) {
                return -267;
            }
            return 1;
        }
    }

after_verify:
    current_seq = (current_seq + 1) & 0xFF;
    {
        s32 pass;
        s32* map = block_map_ptr;

        for (pass = 0; pass < 2; pass++, map += 64) {
            s32 remaining = file_size;

            data = U32_TO_PTR(u8*, arg2);
            for (i = 0; i < file_blocks && remaining > 0; i++) {
                s32 phys;
                s32 chunk;

                if ((u32) remaining > (u32) (chunk = state->x8 - 0x20)) {
                    phys = map[i];
                    if (phys >= 0) {
                        if (arg3 != 0) {
                            s32 zero;
                            s32 cmd_result;
                            CardCmd init_cmd;
                            s32 ofs;
                            CardCmd cmd;

                            ofs = fn_803ACBE8(arg0, phys);
                            if (phys == 0) {
                                zero = 0;
                                if (blocks_before + i != 0) {
                                    cmd_result = -0x101;
                                    goto repair_full_queued;
                                }
                                init_cmd.type = 2;
                                init_cmd.state = PTR_TO_U32(arg0);
                                init_cmd.x10 = zero;
                                init_cmd.x14 = zero;
                                init_cmd.x18 = zero;
                                init_cmd.x20 = zero;
                                init_cmd.x1C = ofs;
                                fn_803AC168((s32*) &init_cmd);
                            }
                            cmd.type = 1;
                            cmd.state = PTR_TO_U32(arg0);
                            cmd.xC = phys;
                            cmd.x10 = blocks_before + i;
                            cmd.x14 = current_seq;
                            cmd.x18 = PTR_TO_U32(data);
                            cmd.x20 = chunk;
                            cmd.x1C = ofs;
                            cmd.x8 = arg1;
                            cmd_result = fn_803AC168((s32*) &cmd);
                        repair_full_queued:
                            if (cmd_result < 0) {
                                fn_803AE7F8_rewind(entries);
                                return cmd_result;
                            }
                        } else {
                            result =
                                fn_803ACFC0(arg0, phys, blocks_before + i,
                                            current_seq, data, chunk, arg1);
                            if (result < 0) {
                                state->x170[block_map[pass][i]] = -0x7FFF;
                                state->x270[block_map[pass][i]] = 0;
                                fn_803AE7F8_close(state);
                                return pass == 0 ? -260 : -258;
                            }
                            state->x170[map[i]] = blocks_before + i;
                            state->x270[map[i]] = current_seq;
                        }
                    }

                    remaining -= state->x8 - 0x20;
                    data += state->x8 - 0x20;
                } else {
                    phys = map[i];
                    if (phys >= 0) {
                        if (arg3 != 0) {
                            s32 zero;
                            s32 cmd_result;
                            CardCmd init_cmd;
                            s32 ofs;
                            CardCmd cmd;

                            ofs = fn_803ACBE8(arg0, phys);
                            if (phys == 0) {
                                zero = 0;
                                if (blocks_before + i != 0) {
                                    cmd_result = -0x101;
                                    goto repair_tail_queued;
                                }
                                init_cmd.type = 2;
                                init_cmd.state = PTR_TO_U32(arg0);
                                init_cmd.x10 = zero;
                                init_cmd.x14 = zero;
                                init_cmd.x18 = zero;
                                init_cmd.x20 = zero;
                                init_cmd.x1C = ofs;
                                fn_803AC168((s32*) &init_cmd);
                            }
                            cmd.type = 1;
                            cmd.state = PTR_TO_U32(arg0);
                            cmd.xC = phys;
                            cmd.x10 = blocks_before + i;
                            cmd.x14 = current_seq;
                            cmd.x18 = PTR_TO_U32(data);
                            cmd.x20 = remaining;
                            cmd.x1C = ofs;
                            cmd.x8 = arg1;
                            cmd_result = fn_803AC168((s32*) &cmd);
                        repair_tail_queued:
                            if (cmd_result < 0) {
                                fn_803AE7F8_rewind(entries);
                                return cmd_result;
                            }
                        } else {
                            result = fn_803ACFC0(arg0, phys, blocks_before + i,
                                                 current_seq, data, remaining,
                                                 arg1);
                            if (result < 0) {
                                state->x170[block_map[pass][i]] = -0x7FFF;
                                state->x270[block_map[pass][i]] = 0;
                                fn_803AE7F8_close(state);
                                return pass == 0 ? -260 : -258;
                            }
                            state->x170[map[i]] = blocks_before + i;
                            state->x270[map[i]] = current_seq;
                        }
                    }

                    remaining = 0;
                }
            }
        }
    }

    if (arg3 == 0) {
        s32 retries;
        for (retries = 0; retries < 10; retries++) {
            result = CARDClose(&state->file_info);
            if (result != -1) {
                break;
            }
        }

        if (result < 0) {
            return repair_result == 0 ? -267 : repair_result;
        }
    } else {
        CardBufEntry* entry = entries;

        entry->x0 = 2;
        entry->x4 = PTR_TO_U32(arg0);
        entry->x8 = arg4;
        entry->xC = arg1;
        hsd_804D7998 = -1;
    }

    return repair_result;
}

static inline s32 fn_803AF3F0_chunk_size(CardState* state)
{
    return state->x8 - 0x20;
}

static inline s32 fn_803AF3F0_queue_verify_first(CardState* state, s32 block,
                                                 s32 logical, s32 seq,
                                                 void* data, s32 size)
{
    if (size == 0) {
        return 0;
    }
    {
        CardCmd cmd;
        s32 ofs = fn_803ACBE8(state, block);
        cmd.type = 5;
        cmd.state = PTR_TO_U32(state);
        cmd.x10 = logical;
        cmd.x14 = seq;
        cmd.x18 = PTR_TO_U32(data);
        cmd.x20 = size;
        cmd.x1C = ofs;
        return fn_803AC168((s32*) &cmd);
    }
}

static inline s32 fn_803AF3F0_queue_verify_final(CardState* state, s32 block,
                                                 s32 logical, s32 seq,
                                                 void* data, s32 size)
{
    s32 result;

    if (size == 0) {
        return 0;
    }
    {
        CardCmd cmd;
        s32 ofs = fn_803ACBE8(state, block);
        cmd.type = 5;
        cmd.state = PTR_TO_U32(state);
        cmd.x10 = logical;
        cmd.x14 = seq;
        cmd.x18 = PTR_TO_U32(data);
        cmd.x20 = size;
        cmd.x1C = ofs;
        result = fn_803AC168((s32*) &cmd);
    }
    return result;
}

static inline s32 fn_803AF3F0_queue_write_first(CardState* state, s32 block,
                                                s32 logical, s32 seq,
                                                void* data, s32 size,
                                                s32 file_id)
{
    CardCmd cmd;
    s32 ofs = fn_803ACBE8(state, block);
    CardCmd init_cmd;

    if (block == 0) {
        s32 zero = 0;

        if (logical != 0) {
            return -0x101;
        }
        init_cmd.type = 2;
        init_cmd.state = PTR_TO_U32(state);
        init_cmd.x10 = zero;
        init_cmd.x14 = zero;
        init_cmd.x18 = zero;
        init_cmd.x20 = zero;
        init_cmd.x1C = ofs;
        fn_803AC168((s32*) &init_cmd);
    }
    cmd.type = 1;
    cmd.state = PTR_TO_U32(state);
    cmd.xC = block;
    cmd.x10 = logical;
    cmd.x14 = seq;
    cmd.x18 = PTR_TO_U32(data);
    cmd.x20 = size;
    cmd.x1C = ofs;
    cmd.x8 = file_id;
    return fn_803AC168((s32*) &cmd);
}

static inline s32 fn_803AF3F0_queue_write_final(CardState* state, s32 block,
                                                s32 logical, s32 seq,
                                                void* data, s32 size,
                                                s32 file_id)
{
    CardCmd cmd;
    s32 ofs = fn_803ACBE8(state, block);
    CardCmd init_cmd;
    s32 result;

    if (block == 0) {
        s32 zero = 0;

        if (logical != 0) {
            return -0x101;
        }
        init_cmd.type = 2;
        init_cmd.state = PTR_TO_U32(state);
        init_cmd.x10 = zero;
        init_cmd.x14 = zero;
        init_cmd.x18 = zero;
        init_cmd.x20 = zero;
        init_cmd.x1C = ofs;
        fn_803AC168((s32*) &init_cmd);
    }
    cmd.type = 1;
    cmd.state = PTR_TO_U32(state);
    cmd.xC = block;
    cmd.x10 = logical;
    cmd.x14 = seq;
    cmd.x18 = PTR_TO_U32(data);
    cmd.x20 = size;
    cmd.x1C = ofs;
    cmd.x8 = file_id;
    result = fn_803AC168((s32*) &cmd);
    return result;
}

static inline void fn_803AF3F0_rewind(CardBufEntry* entries)
{
    s32 snap = hsd_804D7998;

    if (snap >= 0) {
        s32 saved = snap;
        s32 zero;

        while (saved != hsd_804D7984) {
            CardBufEntry* queued = &entries[saved];
            saved = (saved + 1) % 128;
            queued->x10 = zero = 0;
        }
        hsd_804D7984 = snap;
    }
}

static inline void fn_803AF3F0_close(CardState* state)
{
    s32 retries;

    for (retries = 0; retries < 10; retries++) {
        if (CARDClose(&state->file_info) != -1) {
            break;
        }
    }
}

static inline s32 fn_803AF3F0_open(s32 fd, s32 ofs, CARDFileInfo* info)
{
    s32 retries;
    s32 open_result;

    for (retries = 0; retries < 10; retries++) {
        open_result = CARDFastOpen(fd, ofs, info);
        if (open_result != -1) {
            break;
        }
    }
    return open_result;
}

static inline void fn_803AF3F0_check_seq(CardState* state, const s32* primary,
                                         s32 count, s32 current_seq,
                                         s32* seq_match)
{
    s32 i;

    for (i = 0; i < count; i++) {
        if (primary[i] < 0 || state->x270[primary[i]] != current_seq) {
            *seq_match = 1;
            break;
        }
    }
}

static inline void fn_803AF3F0_calc_file_blocks(s32 file_idx, CardState* state,
                                                s32* file_blocks,
                                                s32* total_blocks)
{
    if (state->x4C[file_idx] <= 0) {
        *file_blocks = 0;
    } else if (file_idx == 0) {
        u32 sector_size = state->x8;
        u32 usable;
        s32 rem;

        rem = state->x4C[0];
        usable = sector_size - 0x20;
        rem = rem - (s32) (usable - (state->x24 + 0x30) % sector_size);
        if (rem <= 0) {
            *file_blocks = 1;
        } else {
            *file_blocks =
                (u32) (rem + sector_size - 0x21) / (sector_size - 0x20) + 1;
        }
    } else {
        u32 sector_size = state->x8;
        *file_blocks = (u32) (state->x4C[file_idx] + sector_size - 0x21) /
                       (sector_size - 0x20);
    }

    *total_blocks = fn_803AC7DC(state);
}

s32 fn_803AF3F0(CardState* state, s32 arg1, s32 arg2, s32 arg3, s32 arg4)
{
    CardBufEntry* entries = (CardBufEntry*) hsd_804D1138;
    s32 block_map[3][64];
    s32 seq_match;
    s32 blocks_before;
    s32 file_blocks;
    s32 file_size;
    s32 result;
    s32 current_seq;
    s32 secondary_count;
    s32 free_count;
    s32 i;
    s32 remaining;
    s32 total_blocks;
    u8* data;
    struct {
        s32 v;
    } free_blk;
    struct {
        s32 v;
    } write_blk;
    PAD_STACK(16);

    seq_match = 0;
    if (arg3 == 0) {
        BOOL intr = OSDisableInterrupts();
        s32 busy;
        if (hsd_804D799C == 1) {
            busy = 1;
        } else {
            busy = 0;
        }
        OSRestoreInterrupts(intr);
        if (busy) {
            return -264;
        }
    }

    blocks_before = fn_803AC6B8_blocks_before(state, arg1);

    file_size = state->x4C[arg1];
    fn_803AF3F0_calc_file_blocks(arg1, state, &file_blocks, &total_blocks);

    if (arg3 != 0) {
        hsd_804D7998 = hsd_804D7984;
    } else {
        s32 open_result =
            fn_803AF3F0_open(state->x4, state->x20, &state->file_info);

        if (open_result < 0) {
            return open_result;
        }
    }

    for (i = 0; i < file_blocks; i++) {
        block_map[0][i] = -1;
        block_map[1][i] = -1;
    }

    current_seq = -1;
    {
        secondary_count = 0;
        free_count = 0;
        for (i = 1; i <= total_blocks; i++) {
            s32 arg1x = state->x170[i];

            if (arg1x >= 0) {
                s32 logical = arg1x - blocks_before;
                if (0 <= logical && logical < file_blocks) {
                    if (fn_803ACB74(current_seq, state->x270[i]) < 0) {
                        current_seq = state->x270[i];
                    }
                    block_map[0][logical] = i;
                }
            } else {
                s32 logical = -(arg1x + blocks_before);
                if (0 <= logical && logical < file_blocks) {
                    block_map[1][secondary_count] = i;
                    secondary_count++;
                } else {
                    block_map[2][free_count] = i;
                    free_count++;
                }
            }
        }
    }

    fn_803AF3F0_check_seq(state, block_map[0], file_blocks, current_seq,
                          &seq_match);

    if (seq_match == 0) {
        remaining = file_size;
        data = U32_TO_PTR(u8*, arg2);
        for (i = 0; i < file_blocks && remaining > 0; i++) {
            if ((u32) remaining > (u32) fn_803AF3F0_chunk_size(state)) {
                if (arg3 != 0) {
                    s32 block = block_map[0][i];
                    s32 cmd_result = fn_803AF3F0_queue_verify_first(
                        state, block, blocks_before + i, current_seq, data,
                        fn_803AF3F0_chunk_size(state));
                    if (cmd_result < 0) {
                        fn_803AF3F0_rewind(entries);
                        return cmd_result;
                    }
                } else {
                    s32 verify_result = fn_803ACC0C(
                        state, block_map[0][i], blocks_before + i, current_seq,
                        data, fn_803AF3F0_chunk_size(state));
                    if (verify_result < 0) {
                        fn_803AF3F0_close(state);
                        return verify_result;
                    }
                    if (verify_result > 0) {
                        goto after_verify;
                    }
                }
                remaining -= fn_803AF3F0_chunk_size(state);
                data += fn_803AF3F0_chunk_size(state);
            } else {
                if (arg3 != 0) {
                    s32 cmd_result = fn_803AF3F0_queue_verify_final(
                        state, block_map[0][i], blocks_before + i, current_seq,
                        data, remaining);
                    if (cmd_result < 0) {
                        fn_803AF3F0_rewind(entries);
                        return cmd_result;
                    }
                } else {
                    s32 verify_result =
                        fn_803ACC0C(state, block_map[0][i], blocks_before + i,
                                    current_seq, data, remaining);
                    if (verify_result < 0) {
                        fn_803AF3F0_close(state);
                        return verify_result;
                    }
                    if (verify_result > 0) {
                        goto after_verify;
                    }
                }
                remaining = 0;
            }
        }

        if (arg3 != 0) {
            CardCmd cmd;
            s32 cmd_result;
            cmd.type = 6;
            cmd.state = PTR_TO_U32(state);
            cmd_result = fn_803AC168((s32*) &cmd);
            if (cmd_result < 0) {
                fn_803AF3F0_rewind(entries);
                return cmd_result;
            }
        } else {
            s32 retries;
            for (retries = 0; retries < 10; retries++) {
                result = CARDClose(&state->file_info);
                if (result != -1) {
                    break;
                }
            }
            if (result < 0) {
                return -267;
            }
            return 1;
        }
    }
after_verify:

    if (secondary_count > file_blocks) {
        for (i = file_blocks; i < secondary_count; i++) {
            if (arg3 != 0) {
                s32 cmd_result;

                free_blk.v = block_map[1][i];
                cmd_result = fn_803AF3F0_queue_write_final(
                    state, free_blk.v, 0xFFFF, 0, NULL, 0, arg1);
                if (cmd_result < 0) {
                    fn_803AF3F0_rewind(entries);
                    return cmd_result;
                }
            } else {
                result = fn_803ACFC0(state, block_map[1][i], 0xFFFF, 0, NULL,
                                     0, arg1);
                state->x170[block_map[1][i]] = -0x7FFF;
                state->x270[block_map[1][i]] = 0;
                if (result < 0) {
                    fn_803AF3F0_close(state);
                    return result;
                }
            }
        }
    } else if (secondary_count < file_blocks) {
        while (secondary_count < file_blocks && free_count > 0) {
            block_map[1][secondary_count++] = block_map[2][--free_count];
        }
        if (secondary_count < file_blocks) {
            for (i = 0; i < file_blocks && secondary_count < file_blocks; i++)
            {
                s32 primary = block_map[0][i];
                if (0 <= primary) {
                    block_map[1][secondary_count++] = primary;
                    block_map[0][i] = -1;
                }
            }
            if (secondary_count < file_blocks) {
                if (arg3 == 0) {
                    fn_803AF3F0_close(state);
                }
                return -257;
            }
        }
    }

    {
        s32 next = current_seq + 1;
        remaining = file_size;
        data = U32_TO_PTR(u8*, arg2);
        current_seq = next & 0xFF;
    }
    for (i = 0; i < file_blocks && remaining > 0; i++) {
        s32 chunk;
        (void) (remaining != 0);

        if ((u32) remaining > (u32) (chunk = fn_803AF3F0_chunk_size(state))) {
            if (arg3 != 0) {
                s32 cmd_result;

                write_blk.v = block_map[1][i];
                cmd_result = fn_803AF3F0_queue_write_first(
                    state, write_blk.v, blocks_before + i, current_seq, data,
                    chunk, arg1);
                if (cmd_result < 0) {
                    fn_803AF3F0_rewind(entries);
                    return cmd_result;
                }
            } else {
                s32 write_result = fn_803ACFC0(
                    state, block_map[1][i], blocks_before + i, current_seq,
                    data, fn_803AF3F0_chunk_size(state), arg1);
                if (write_result < 0) {
                    state->x170[block_map[1][i]] = -0x7FFF;
                    state->x270[block_map[1][i]] = 0;
                    fn_803AF3F0_close(state);
                    return write_result;
                }
                state->x170[block_map[1][i]] = blocks_before + i;
                state->x270[block_map[1][i]] = current_seq;
            }
            remaining -= state->x8 - 0x20;
            data += fn_803AF3F0_chunk_size(state);
        } else {
            if (arg3 != 0) {
                s32 block = block_map[1][i];
                s32 cmd_result = fn_803AF3F0_queue_write_final(
                    state, block, blocks_before + i, current_seq, data,
                    remaining, arg1);
                if (cmd_result < 0) {
                    fn_803AF3F0_rewind(entries);
                    return cmd_result;
                }
            } else {
                s32 write_result =
                    fn_803ACFC0(state, block_map[1][i], blocks_before + i,
                                current_seq, data, remaining, arg1);
                if (write_result < 0) {
                    state->x170[block_map[1][i]] = -0x7FFF;
                    state->x270[block_map[1][i]] = 0;
                    fn_803AF3F0_close(state);
                    return write_result;
                }
                state->x170[block_map[1][i]] = blocks_before + i;
                state->x270[block_map[1][i]] = current_seq;
            }
            remaining = 0;
        }
    }

    if (arg3 == 0) {
        for (i = 0; i < file_blocks; i++) {
            if (block_map[0][i] >= 0) {
                state->x170[block_map[0][i]] = -state->x170[block_map[0][i]];
            }
        }
        {
            s32 retries;
            s32 close_result;

            for (retries = 0; retries < 10; retries++) {
                close_result = CARDClose(&state->file_info);
                if (close_result != -1) {
                    break;
                }
            }
            if (close_result < 0) {
                return -267;
            }
        }
    } else {
        CardBufEntry* entry = entries;

        entry->x0 = 3;
        entry->x4 = PTR_TO_U32(state);
        entry->x8 = arg4;
        entry->xC = arg1;
        hsd_804D7998 = -1;
    }

    return 0;
}

static inline s32 fn_803B0120_first_chunk(CardState* state)
{
    u32 sector_size = state->x8;

    return (s32) (sector_size - 0x20) -
           (s32) ((u32) (state->x24 + 0x30) % sector_size);
}

static inline void fn_803B0120_rewind(CardBufEntry* entries)
{
    s32 snap = hsd_804D7998;

    if (snap >= 0) {
        s32 saved = snap;
        s32 zero;

        while (saved != hsd_804D7984) {
            CardBufEntry* queued = &entries[saved];
            saved = (saved + 1) % 128;
            queued->x10 = zero = 0;
        }
        hsd_804D7984 = snap;
    }
}

static inline void fn_803B0120_close(CardState* state)
{
    s32 retries;

    for (retries = 0; retries < 10; retries++) {
        if (CARDClose(&state->file_info) != -1) {
            break;
        }
    }
}

static inline s32 fn_803B0120_close_result(CardState* state)
{
    s32 result;
    s32 retries;
    s32 ret;

    for (retries = 0; retries < 10; retries++) {
        result = CARDClose(&state->file_info);
        if (result != -1) {
            break;
        }
    }
    ret = result;
    return ret;
}

static inline s32 fn_803B0120_block_offset(CardState* state, s32 block_idx)
{
    u32 size = state->x8;
    u32 temp = state->x24 + size;
    u32 num = temp + 0x2F;
    u32 idx = num / size - 1;
    u32 pos = block_idx + idx;

    (void) temp;
    (void) idx;
    return size * pos;
}

static inline s32 fn_803B0120_queue_verify(CardState* state, s32 block,
                                           s32 logical, s32 seq, void* data,
                                           s32 size)
{
    s32 result;

    if (size == 0) {
        return 0;
    }
    {
        CardCmd cmd;
        s32 ofs = fn_803B0120_block_offset(state, block);
        cmd.type = 5;
        cmd.state = PTR_TO_U32(state);
        cmd.x10 = logical;
        cmd.x14 = seq;
        cmd.x18 = PTR_TO_U32(data);
        cmd.x20 = size;
        cmd.x1C = ofs;
        result = fn_803AC168((s32*) &cmd);
    }
    return result;
}

static inline s32 fn_803B0120_queue_write(CardState* state, s32 block,
                                          s32 logical, s32 seq, void* data,
                                          s32 size, s32 file_id)
{
    CardCmd cmd;
    s32 ofs = fn_803ACBE8(state, block);
    CardCmd init_cmd;
    s32 result;

    if (block == 0) {
        s32 zero = 0;

        if (logical != 0) {
            return -0x101;
        }
        init_cmd.type = 2;
        init_cmd.state = PTR_TO_U32(state);
        init_cmd.x10 = zero;
        init_cmd.x14 = zero;
        init_cmd.x18 = zero;
        init_cmd.x20 = zero;
        init_cmd.x1C = ofs;
        fn_803AC168((s32*) &init_cmd);
    }
    cmd.type = 1;
    cmd.state = PTR_TO_U32(state);
    cmd.xC = block;
    cmd.x10 = logical;
    cmd.x14 = seq;
    cmd.x18 = PTR_TO_U32(data);
    cmd.x20 = size;
    cmd.x1C = ofs;
    cmd.x8 = file_id;
    result = fn_803AC168((s32*) &cmd);
    return result;
}

s32 fn_803B0120(CardState* state, s32 arg1, s32 arg2, s32 arg3, s32 arg4)
{
    CardBufEntry* entries = (CardBufEntry*) hsd_804D1138;
    s32 block_map[3][64];
    s32 needs_rewrite;
    s32 blocks_before;
    s32 file_blocks;
    s32 file_size;
    s32 total_blocks;
    s32 current_seq;
    s32 file_idx;
    s32 seq;
    s32 i;
    s32 secondary_count;
    s32 free_count;
    s32 logical;
    s32 remaining;
    s32 result;
    u8* data;
    s32 j;
    PAD_STACK(16);

    needs_rewrite = 0;
    if (arg3 == 0) {
        BOOL intr = OSDisableInterrupts();
        s32 busy;
        if (hsd_804D799C == 1) {
            busy = 1;
        } else {
            busy = 0;
        }
        OSRestoreInterrupts(intr);
        if (busy) {
            return -264;
        }
    }

    blocks_before = fn_803AC6B8_blocks_before(state, arg1);

    {
        s32 size = state->x4C[arg1];
        file_size = size;
    }
    file_blocks = calculateFileBlockCount(state, arg1);
    total_blocks = fn_803AC7DC(state);

    if (arg3 != 0) {
        hsd_804D7998 = hsd_804D7984;
    } else {
        s32 open_result =
            fn_803AF3F0_open(state->x4, state->x20, &state->file_info);
        if (open_result < 0) {
            return open_result;
        }
    }

    for (j = 0; j < file_blocks; j++) {
        block_map[0][j] = -1;
        block_map[1][j] = -1;
    }

    current_seq = -1;
    if (arg1 == 0) {
        if (state->x170[0] == 0) {
            current_seq = state->x270[0];
        } else {
            needs_rewrite = 1;
        }
        block_map[0][0] = 0;
    }

    {
        secondary_count = 0;
        free_count = 0;
        for (i = 1; i <= total_blocks; i++) {
            file_idx = state->x170[i];

            if (file_idx >= 0) {
                logical = file_idx - blocks_before;
                if (arg1 == 0 && logical == 0) {
                    block_map[1][secondary_count] = i;
                    secondary_count++;
                } else if (logical >= 0 && logical < file_blocks) {
                    seq = state->x270[i];
                    if (fn_803ACB74(current_seq, seq) < 0) {
                        current_seq = seq;
                    }
                    block_map[0][logical] = i;
                }
            } else {
                logical = -(file_idx + blocks_before);
                if (logical >= 0 && logical < file_blocks) {
                    block_map[1][secondary_count] = i;
                    secondary_count++;
                } else {
                    block_map[2][free_count] = i;
                    free_count++;
                }
            }
        }
    }

    {
        for (i = 0; i < file_blocks; i++) {
            if (block_map[0][i] < 0) {
                needs_rewrite = 1;
                if (secondary_count > 0) {
                    secondary_count--;
                    block_map[0][i] = block_map[1][secondary_count];
                    block_map[1][secondary_count] = -1;
                } else if (free_count > 0) {
                    free_count--;
                    block_map[0][i] = block_map[2][free_count];
                    block_map[2][free_count] = -1;
                } else {
                    if (arg3 == 0) {
                        fn_803B0120_close(state);
                    }
                    return -257;
                }
            }
        }
    }

    if (needs_rewrite == 0) {
        for (i = 0; i < file_blocks; i++) {
            if (block_map[0][i] < 0 ||
                state->x270[block_map[0][i]] != current_seq)
            {
                needs_rewrite = 1;
                break;
            }
        }
    }

    if (needs_rewrite == 0) {
        remaining = file_size;
        data = U32_TO_PTR(u8*, arg2);
        for (i = 0; i < file_blocks && remaining > 0; i++) {
            s32 logical = blocks_before + i;
            s32 chunk;
            if (logical == 0) {
                chunk = fn_803B0120_first_chunk(state);
            } else {
                chunk = state->x8 - 0x20;
            }

            if (remaining > chunk) {
                if (arg3 != 0) {
                    s32 cmd_result = fn_803B0120_queue_verify(
                        state, block_map[0][i], blocks_before + i, current_seq,
                        data, chunk);
                    if (cmd_result < 0) {
                        fn_803B0120_rewind(entries);
                        return cmd_result;
                    }
                } else {
                    s32 verify_result =
                        fn_803ACC0C(state, block_map[0][i], blocks_before + i,
                                    current_seq, data, chunk);
                    result = verify_result;
                    if (result < 0) {
                        fn_803B0120_close(state);
                        return result;
                    }
                    if (result > 0) {
                        goto verify_done;
                    }
                }
                remaining -= chunk;
                data += chunk;
            } else {
                if (arg3 != 0) {
                    s32 cmd_result = fn_803B0120_queue_verify(
                        state, block_map[0][i], blocks_before + i, current_seq,
                        data, remaining);
                    if (cmd_result < 0) {
                        fn_803B0120_rewind(entries);
                        return cmd_result;
                    }
                } else {
                    result =
                        fn_803ACC0C(state, block_map[0][i], blocks_before + i,
                                    current_seq, data, remaining);
                    if (result < 0) {
                        fn_803B0120_close(state);
                        return result;
                    }
                    if (result > 0) {
                        goto verify_done;
                    }
                }
                remaining = 0;
            }
        }

        if (arg3 != 0) {
            CardCmd cmd;
            s32 cmd_result;
            cmd.type = 6;
            cmd.state = PTR_TO_U32(state);
            cmd_result = fn_803AC168((s32*) &cmd);
            if (cmd_result < 0) {
                fn_803B0120_rewind(entries);
                return cmd_result;
            }
        } else {
            s32 retries;
            for (retries = 0; retries < 10; retries++) {
                result = CARDClose(&state->file_info);
                if (result != -1) {
                    break;
                }
            }
            if (result < 0) {
                return -267;
            }
            return 1;
        }

    verify_done:;
    }
    while (secondary_count > 0) {
        secondary_count--;
        if (arg3 != 0) {
            s32 block = block_map[1][secondary_count];
            s32 cmd_result = fn_803B0120_queue_write(state, block, 0xFFFF, 0,
                                                     NULL, 0, arg1);
            if (cmd_result < 0) {
                fn_803B0120_rewind(entries);
                return cmd_result;
            }
        } else {
            s32* secondary = block_map[1];
            s32* block = &secondary[secondary_count];
            result = fn_803ACFC0(state, *block, 0xFFFF, 0, NULL, 0, arg1);
            state->x170[*block] = -0x7FFF;
            state->x270[*block] = 0;
            if (result < 0) {
                fn_803B0120_close(state);
            }
            return result;
        }
    }

    {
        s32 next = current_seq + 1;
        remaining = file_size;
        data = U32_TO_PTR(u8*, arg2);
        seq = next & 0xFF;
    }
    for (i = 0; i < file_blocks && remaining > 0; i++) {
        s32 logical = blocks_before + i;
        s32 chunk;
        if (logical == 0) {
            chunk = fn_803B0120_first_chunk(state);
        } else {
            chunk = state->x8 - 0x20;
        }

        if (remaining > chunk) {
            if (arg3 != 0) {
                s32 block = block_map[0][i];
                s32 cmd_result = fn_803B0120_queue_write(
                    state, block, logical, seq, data, chunk, arg1);
                if (cmd_result < 0) {
                    fn_803B0120_rewind(entries);
                    return cmd_result;
                }
            } else {
                s32 write_result =
                    fn_803ACFC0(state, block_map[0][i], blocks_before + i, seq,
                                data, chunk, arg1);
                result = write_result;
                if (result < 0) {
                    state->x170[block_map[0][i]] = -0x7FFF;
                    state->x270[block_map[0][i]] = 0;
                    fn_803B0120_close(state);
                    return result;
                }
                state->x170[block_map[0][i]] = blocks_before + i;
                state->x270[block_map[0][i]] = seq;
            }
            remaining -= chunk;
            data += chunk;
        } else {
            if (arg3 != 0) {
                s32 block = block_map[0][i];
                s32 cmd_result = fn_803B0120_queue_write(
                    state, block, logical, seq, data, remaining, arg1);
                if (cmd_result < 0) {
                    fn_803B0120_rewind(entries);
                    return cmd_result;
                }
            } else {
                s32 write_result =
                    fn_803ACFC0(state, block_map[0][i], blocks_before + i, seq,
                                data, remaining, arg1);
                result = write_result;
                if (result < 0) {
                    state->x170[block_map[0][i]] = -0x7FFF;
                    state->x270[block_map[0][i]] = 0;
                    fn_803B0120_close(state);
                    return result;
                }
                state->x170[block_map[0][i]] = blocks_before + i;
                state->x270[block_map[0][i]] = seq;
            }
            remaining = 0;
        }
    }

    if (arg3 == 0) {
        result = fn_803B0120_close_result(state);
        if (result < 0) {
            return -267;
        }
    } else {
        entries[0].x0 = 4;
        entries[0].x4 = PTR_TO_U32(state);
        entries[0].x8 = arg4;
        entries[0].xC = arg1;
        hsd_804D7998 = -1;
    }
    PAD_STACK(4);

    return 0;
}

static inline s32 fn_803B0E9C_read_icons(struct CardState* arg0, CardCmd* cmd,
                                         s32 file_id, s32 seq)
{
    u32 block_idx;
    s32 result;

    for (block_idx = 0; block_idx < (0x2F + arg0->x24 + arg0->x8) / arg0->x8;
         block_idx++)
    {
        cmd->type = 10;
        cmd->state = PTR_TO_U32(arg0);
        cmd->xC = file_id;
        cmd->x10 = seq;
        cmd->x8 = block_idx;
        result = fn_803AC168((s32*) cmd);
        if (result < 0) {
            return result;
        }
    }
    return 0;
}

static inline s32 fn_803B0E9C_write_block(struct CardState* arg0,
                                          s32 block_idx)
{
    u32 write_size = arg0->x8;
    s32 offset = write_size * block_idx;
    u8* buf = CARD_BUF(arg0);
    s32 retries;
    s32 result;

    for (retries = 0; retries < 10; retries++) {
        result = CARDWrite(&arg0->file_info, buf, write_size, offset);
        if (result != -1) {
            break;
        }
    }
    return result;
}

static inline s32 fn_803B0E9C_write_block_final(struct CardState* arg0,
                                                s32 block_idx)
{
    u32 write_size = arg0->x8;
    s32 retries;
    s32 result;
    s32 offset = write_size * block_idx;
    u8* buf = CARD_BUF(arg0);

    for (retries = 0; retries < 10; retries++) {
        result = CARDWrite(&arg0->file_info, buf, write_size, offset);
        if (result != -1) {
            break;
        }
    }
    return result;
}

static inline s32 fn_803B0E9C_read_first(struct CardState* arg0,
                                         u32 sector_size)
{
    u32 read_size = sector_size;
    u8* buf = CARD_BUF(arg0);
    s32 retries;
    s32 result;

    for (retries = 0; retries < 10; retries++) {
        result = CARDRead(&arg0->file_info, buf, read_size, 0);
        if (result != -1) {
            break;
        }
    }
    return result;
}

s32 fn_803B0E9C(struct CardState* arg0, s32 arg1, s32 arg2, s32 arg3, s32 arg4)
{
    u8 digest[0x30];
    CardCmd cmd_done;
    CardCmd cmd_clear;
    UNUSED u8 pad_cmd_clear[4];
    CardCmd cmd_patch;
    UNUSED u8 pad_cmd_patch[8];
    CardCmd cmd_write_icon;
    UNUSED u8 pad_cmd_write_icon[16];
    CardCmd cmd_read_icon;
    UNUSED u8 pad_cmd_read_icon[8];
    s32 remaining;
    s32 payload_pos;
    s32 block_idx;
    s32 result;
    s32 has_blocks;
    u32 sector_size;

    arg0->x24 = hsd_803AC340(&arg0->x3B0);

    if (arg3 == 0) {
        if (arg4 != 0) {
            result = fn_803B0E9C_read_icons(arg0, &cmd_read_icon, arg1, arg2);
            if (result < 0) {
                return result;
            }

            {
                cmd_done.type = 6;
                cmd_done.state = PTR_TO_U32(arg0);
                result = fn_803AC168((s32*) &cmd_done);
            }
            if (result < 0) {
                return result;
            }
        } else {
            result = fn_803ACD58(arg0, U32_TO_PTR(void*, arg1), U32_TO_PTR(void*, arg2));
            if (result < 0) {
                return result;
            }
            if (result == 0) {
                return 1;
            }
        }
    }

    if (arg4 != 0) {
        for (arg4 = 0; sector_size = arg0->x8,
            (u32) arg4 < (0x2F + arg0->x24 + sector_size) / sector_size;
             arg4++)
        {
            if ((u32) (arg4 + 1) ==
                (0x2F + arg0->x24 + sector_size) / sector_size)
            {
                if (arg3 != 0) {
                    void* clear_buf = CARD_BUF(arg0);
                    s32 zero = 0;
                    cmd_clear.type = 4;
                    cmd_clear.state = PTR_TO_U32(arg0);
                    cmd_clear.x10 = zero;
                    cmd_clear.x14 = zero;
                    cmd_clear.x18 = PTR_TO_U32(clear_buf);
                    cmd_clear.x20 = sector_size;
                    cmd_clear.x1C = zero;
                    cmd_clear.x8 = zero;
                    result = fn_803AC168((s32*) &cmd_clear);
                } else {
                    if (arg0->x4C[0] > 0) {
                        has_blocks = 1;
                    } else {
                        has_blocks = 0;
                    }
                    cmd_patch.type = 2;
                    cmd_patch.state = PTR_TO_U32(arg0);
                    cmd_patch.x10 = -1;
                    cmd_patch.x14 = 0;
                    cmd_patch.x18 = 0;
                    cmd_patch.x20 = has_blocks;
                    cmd_patch.x1C = arg4 * sector_size;
                    result = fn_803AC168((s32*) &cmd_patch);
                }
                if (result < 0) {
                    return result;
                }
            }

            {
                cmd_write_icon.type = 9;
                cmd_write_icon.state = PTR_TO_U32(arg0);
                cmd_write_icon.x8 = arg4;
                cmd_write_icon.xC = arg1;
                cmd_write_icon.x10 = arg2;
                result = fn_803AC168((s32*) &cmd_write_icon);
                if (result < 0) {
                    return result;
                }
            }
        }
        return 0;
    }

    sector_size = arg0->x8;
    if (arg0->x24 + 0x30 < sector_size) {
        if (arg3 != 0) {
            memset(CARD_BUF(arg0), 0, sector_size);
        } else {
            result = fn_803B0E9C_read_first(arg0, sector_size);
            if (result < 0) {
                return result;
            }
        }
    }

    memcpy(CARD_BUF(arg0), arg0->x370, 0x40);
    payload_pos = 0x40;
    switch (arg0->x3B0) {
    case 2: {
        void* dst = CARD_BUF(arg0) + 0x40;
        memcpy(dst, U32_TO_PTR(void*, arg1), 0x1800);
    }
        payload_pos = 0x1840;
        break;
    case 1: {
        void* dst = CARD_BUF(arg0) + 0x40;
        memcpy(dst, U32_TO_PTR(void*, arg1), 0xE00);
    }
        payload_pos = 0xE40;
        break;
    }

    memset(digest, 0, sizeof(digest));
    block_idx = 0;
    remaining = arg0->x24 - payload_pos;

    while (remaining >= 0) {
        sector_size = arg0->x8;
        if ((u32) (payload_pos + remaining) > sector_size) {
            memcpy(CARD_BUF(arg0) + payload_pos, U32_TO_PTR(s32*, arg2),
                   sector_size - payload_pos);
            sector_size = arg0->x8;
            arg2 += sector_size - payload_pos;
            remaining -= sector_size - payload_pos;
            hsd_803B2B20(CARD_BUF(arg0), sector_size, &digest[block_idx * 0x10]);

            result = fn_803B0E9C_write_block(arg0, block_idx);

            if (result < 0) {
                return result;
            }

            payload_pos = 0;
            block_idx++;

            if (remaining + 0x30 < (s32) arg0->x24) {
                if (arg3 != 0) {
                    memset(CARD_BUF(arg0), 0, arg0->x8);
                } else {
                    u32 read_size = arg0->x8;
                    s32 retries2;
                    u8* buf = CARD_BUF(arg0);

                    for (retries2 = 0; retries2 < 10; retries2++) {
                        result = CARDRead(&arg0->file_info, buf, read_size,
                                          read_size * block_idx);
                        if (result != -1) {
                            break;
                        }
                    }

                    if (result < 0) {
                        return result;
                    }
                }
            }
            continue;
        }

        memcpy(CARD_BUF(arg0) + payload_pos, U32_TO_PTR(void*, arg2), remaining);
        payload_pos += remaining;
        hsd_803B2B20(CARD_BUF(arg0), payload_pos, &digest[block_idx * 0x10]);
        memcpy(CARD_BUF(arg0) + payload_pos, digest, 0x30);
        remaining = -1;

        result = fn_803B0E9C_write_block_final(arg0, block_idx);

        if (result < 0) {
            return result;
        }

        block_idx++;
    }

    return 0;
}

static inline s32 fn_803B1338_queue_write(CardState* state, s32 logical,
                                          s32 phys, void* data, s32 size,
                                          s32 file_id, CardCmdBuf* init_buf,
                                          CardCmdBuf* buf)
{
    s32 ofs = fn_803ACBE8(state, logical);
    s32 result;

    if (logical == 0) {
        if (phys != 0) {
            return -0x101;
        }
        init_buf->cmd.type = 2;
        init_buf->cmd.state = PTR_TO_U32(state);
        init_buf->cmd.x10 = 0;
        init_buf->cmd.x14 = 0;
        init_buf->cmd.x18 = 0;
        init_buf->cmd.x20 = 0;
        init_buf->cmd.x1C = ofs;
        fn_803AC168(init_buf->words);
    }
    buf->cmd.type = 1;
    buf->cmd.state = PTR_TO_U32(state);
    buf->cmd.xC = logical;
    buf->cmd.x10 = phys;
    buf->cmd.x14 = 0;
    buf->cmd.x18 = PTR_TO_U32(data);
    buf->cmd.x20 = size;
    buf->cmd.x1C = ofs;
    buf->cmd.x8 = file_id;
    result = fn_803AC168(buf->words);
    return result;
}

static inline u8* fn_803B1338_data_at(u8* file_payload, s32 offset)
{
    return file_payload + offset;
}

static inline s32 fn_803B1338_data_size(CardState* state)
{
    return state->x8 - 0x20;
}

static inline s32 fn_803B1338_queue_write_data(CardState* state, s32 logical,
                                               s32 phys, u8* fdata, s32 offset,
                                               s32 file_id,
                                               CardCmdBuf* init_buf,
                                               CardCmdBuf* buf)
{
    s32 ofs = fn_803ACBE8(state, logical);
    u8* data = fn_803B1338_data_at(fdata, offset);
    s32 size = fn_803B1338_data_size(state);
    s32 result;

    if (logical == 0) {
        if (phys != 0) {
            return -0x101;
        }
        init_buf->cmd.type = 2;
        init_buf->cmd.state = PTR_TO_U32(state);
        init_buf->cmd.x10 = 0;
        init_buf->cmd.x14 = 0;
        init_buf->cmd.x18 = 0;
        init_buf->cmd.x20 = 0;
        init_buf->cmd.x1C = ofs;
        fn_803AC168(init_buf->words);
    }
    buf->cmd.type = 1;
    buf->cmd.state = PTR_TO_U32(state);
    buf->cmd.xC = logical;
    buf->cmd.x10 = phys;
    buf->cmd.x14 = 0;
    buf->cmd.x18 = PTR_TO_U32(data);
    buf->cmd.x20 = size;
    buf->cmd.x1C = ofs;
    buf->cmd.x8 = file_id;
    result = fn_803AC168(buf->words);
    return result;
}

s32 fn_803B1338(CardState* state, s32 arg1)
{
    PAD_STACK(72);
    {
        CardCmdBuf cmd0;
        CardCmdBuf cmd1;
        CardCmdBuf cmd2;
        CardCmdBuf cmd3;
        CardCmdBuf cmd4;
        CardCmdBuf cmd5;
        CardCmdBuf cmd6;
        CardCmdBuf cmd7;
        CardCmdBuf cmd8;
        CardCmdBuf cmd9;
        CardCmdBuf cmd10;
        CardCmdBuf cmd11;
        CardCmdBuf cmd12;
        CardCmdBuf cmd13;
        CardCmdBuf cmd14;
        CardCmdBuf cmd15;
        CardCmdBuf cmd16;
        CardCmdBuf cmd17;
        s32 result;
        s32 logical;
        s32 max_redun;
        s32 file_blocks;
        s32 index;
        s32 phys;
        u8* fdata;
        PAD_STACK(40);

        max_redun = 0;
        phys = 1;
        logical = 1;

        if ((s32) fn_803AC634(state, 0) != 0) {
            s32 i;
            s32 offset;
            s32 hdr_offset;
            u32 hdr_base;
            u32 sector_size;
            file_blocks = fn_803AC634(state, 0);
            offset = 0;
            sector_size = state->x8;
            hdr_base = state->x24 + 0x30;
            hdr_offset = hdr_base % sector_size;
            if (arg1 != 0) {
                fdata = U32_TO_PTR(u8*, state->x70[0].ptr);
                if (fdata == NULL) {
                    s32 ofs0 =
                        sector_size *
                        ((hdr_base + sector_size - 1) / sector_size - 1);
                    cmd0.cmd.type = 2;
                    cmd0.cmd.state = PTR_TO_U32(state);
                    cmd0.cmd.x10 = 0;
                    cmd0.cmd.x14 = 0;
                    cmd0.cmd.x18 = 0;
                    cmd0.cmd.x20 = 0;
                    cmd0.cmd.x1C = ofs0;
                    fn_803AC168(cmd0.words);
                    cmd1.cmd.type = 1;
                    cmd1.cmd.state = PTR_TO_U32(state);
                    cmd1.cmd.xC = 0;
                    cmd1.cmd.x10 = 0;
                    cmd1.cmd.x14 = 0;
                    cmd1.cmd.x18 = 0;
                    cmd1.cmd.x20 = 0;
                    cmd1.cmd.x1C = ofs0;
                    cmd1.cmd.x8 = 0;
                    result = fn_803AC168(cmd1.words);
                } else {
                    s32 ofs0 =
                        sector_size *
                        ((hdr_base + sector_size - 1) / sector_size - 1);
                    cmd2.cmd.type = 2;
                    cmd2.cmd.state = PTR_TO_U32(state);
                    cmd2.cmd.x10 = 0;
                    cmd2.cmd.x14 = 0;
                    cmd2.cmd.x18 = 0;
                    cmd2.cmd.x20 = 0;
                    cmd2.cmd.x1C = ofs0;
                    fn_803AC168(cmd2.words);
                    cmd3.cmd.type = 1;
                    cmd3.cmd.state = PTR_TO_U32(state);
                    cmd3.cmd.xC = 0;
                    cmd3.cmd.x10 = 0;
                    cmd3.cmd.x14 = 0;
                    cmd3.cmd.x18 = PTR_TO_U32(fdata);
                    cmd3.cmd.x20 = sector_size - hdr_offset - 0x20;
                    cmd3.cmd.x1C = ofs0;
                    cmd3.cmd.x8 = 0;
                    result = fn_803AC168(cmd3.words);
                    offset = state->x8 - hdr_offset - 0x20;
                }
                if (result < 0) {
                    return result;
                }
            } else {
                fdata = U32_TO_PTR(u8*, state->x70[0].ptr);
                if (fdata == NULL) {
                    result = fn_803ACFC0(state, 0, 0, 0, NULL, 0, 0);
                } else {
                    result = fn_803ACFC0(state, 0, 0, 0, fdata,
                                         sector_size - hdr_offset - 0x20, 0);
                    offset = state->x8 - hdr_offset - 0x20;
                }
                if (result < 0 && result != -0x105) {
                    state->x170[0] = -0x7FFF;
                    state->x270[0] = 0;
                    return result;
                }
            }

            for (i = 1; i < file_blocks; i++) {
                if (arg1 != 0) {
                    fdata = U32_TO_PTR(u8*, state->x70[0].ptr);
                    if (fdata == NULL) {
                        result = fn_803B1338_queue_write(
                            state, logical, phys, NULL, 0, 0, &cmd4, &cmd5);
                    } else {
                        s32 chunk;
                        u8* wdata;
                        wdata = fn_803B1338_data_at(fdata, offset);
                        chunk = fn_803B1338_data_size(state);
                        result = fn_803B1338_queue_write(state, logical, phys,
                                                         wdata, chunk, 0,
                                                         &cmd6, &cmd7);
                        offset = state->x8 + offset;
                        offset -= 0x20;
                    }
                    if (result < 0) {
                        return result;
                    }
                } else {
                    fdata = U32_TO_PTR(u8*, state->x70[0].ptr);
                    if (fdata == NULL) {
                        result = CARD_WRITE_BLOCK(state, logical, phys, 0,
                                                  NULL, 0, 0);
                    } else {
                        result = CARD_WRITE_BLOCK(state, logical, phys, 0,
                                                  fdata + offset,
                                                  state->x8 - 0x20, 0);
                        offset = state->x8 + offset;
                        offset -= 0x20;
                    }
                    if (result < 0) {
                        state->x170[logical] = -0x7FFF;
                        state->x270[logical] = 0;
                        return result;
                    }
                    state->x170[logical] = phys;
                    state->x270[logical] = 0;
                }
                logical++;
                phys++;
            }
        }

        for (index = 1; index < 9; index++) {
            s32 j;
            s32 offset;
            file_blocks = fn_803AC7DC_block_count(state, index);
            offset = 0;
            if (file_blocks > 0) {
                for (j = 0; j < file_blocks; j++) {
                    if (arg1 != 0) {
                        fdata = U32_TO_PTR(u8*, state->x70[index].ptr);
                        if (fdata == NULL) {
                            result = fn_803B1338_queue_write(
                                state, logical, phys, NULL, 0, index, &cmd8,
                                &cmd9);
                        } else {
                            result = fn_803B1338_queue_write_data(
                                state, logical, phys, fdata, offset, index,
                                &cmd10, &cmd11);
                            offset = state->x8 + offset;
                            offset -= 0x20;
                        }
                        if (result < 0) {
                            return result;
                        }
                    } else {
                        fdata = U32_TO_PTR(u8*, state->x70[index].ptr);
                        if (fdata == NULL) {
                            result = fn_803ACFC0(state, logical, phys, 0, NULL,
                                                 0, index);
                        } else {
                            result = fn_803ACFC0(state, logical, phys, 0,
                                                 fdata + offset,
                                                 state->x8 - 0x20, index);
                            offset = state->x8 + offset;
                            offset -= 0x20;
                        }
                        if (result < 0) {
                            state->x170[logical] = -0x7FFF;
                            state->x270[logical] = 0;
                            return result;
                        }
                        state->x170[logical] = phys;
                        state->x270[logical] = 0;
                    }
                    logical++;
                    phys++;
                }
                switch (state->x28[index]) {
                case 0:
                    break;
                case 1:
                    if (max_redun < file_blocks) {
                        max_redun = file_blocks;
                    }
                    break;
                case 2:
                    if (max_redun < 1) {
                        max_redun = 1;
                    }
                    break;
                case 3:
                    break;
                }
            }
        }

        {
            s32 i;
            for (i = 0; i < max_redun; i++) {
                if (arg1 != 0) {
                    result =
                        fn_803B1338_queue_write(state, logical, 0xFFFF, NULL,
                                                0, index, &cmd12, &cmd13);
                    if (result < 0) {
                        return result;
                    }
                } else {
                    result =
                        fn_803ACFC0(state, logical, 0xFFFF, 0, NULL, 0, index);
                    state->x170[logical] = -0x7FFF;
                    state->x270[logical] = 0;
                    if (result < 0) {
                        return result;
                    }
                }
                phys++;
                logical++;
            }
        }

        {
            s32 block_idx;
            s32 file_id;

            index = 1;
            if ((s32) fn_803AC634(state, 0) != 0) {
                index += fn_803AC634(state, 0) - 1;
            }
            for (file_id = 1; file_id < 9; file_id++) {
                s32 offset;
                file_blocks = fn_803AC634(state, file_id);
                offset = 0;
                if (file_blocks > 0) {
                    switch (state->x28[file_id]) {
                    case 0:
                        for (block_idx = 0; block_idx < file_blocks;
                             block_idx++)
                        {
                            if (arg1 != 0) {
                                fdata = U32_TO_PTR(u8*, state->x70[file_id].ptr);
                                if (fdata == NULL) {
                                    result = fn_803B1338_queue_write(
                                        state, logical, index, NULL, 0,
                                        file_id, &cmd14, &cmd15);
                                } else {
                                    result = fn_803B1338_queue_write_data(
                                        state, logical, index, fdata, offset,
                                        file_id, &cmd16, &cmd17);
                                    offset = state->x8 + offset;
                                    offset -= 0x20;
                                }
                                if (result < 0) {
                                    return result;
                                }
                            } else {
                                fdata = U32_TO_PTR(u8*, state->x70[file_id].ptr);
                                if (fdata == NULL) {
                                    result = fn_803ACFC0(state, logical, index,
                                                         0, NULL, 0, file_id);
                                } else {
                                    result =
                                        fn_803ACFC0(state, logical, index, 0,
                                                    fdata + offset,
                                                    state->x8 - 0x20, file_id);
                                    offset = state->x8 + offset;
                                    offset -= 0x20;
                                }
                                if (result < 0) {
                                    state->x170[logical] = -0x7FFF;
                                    state->x270[logical] = 0;
                                    return result;
                                }
                                state->x170[logical] = index;
                                state->x270[logical] = 0;
                            }
                            logical++;
                            index++;
                        }
                        break;
                    case 1:
                    case 2:
                    case 3:
                        index += file_blocks;
                        break;
                    }
                }
            }
        }

        return 0;
    }
}

s32 fn_803B1F78(CardState* state, s32 channel, s32 file_id, s32 seq_num,
                s32 callback)
{
    CardCmd buf1;
    CardCmd buf2;
    s32 result;
    s32 blocks;
    s32 saved;
    s32 snap;
    s32 saved1;
    s32 snap1;
    s32 saved2;
    s32 snap2;
    s32 saved3;
    s32 snap3;
    s32 saved4;
    s32 snap4;
    CardBufEntry* entries = (CardBufEntry*) hsd_804D1138;
    PAD_STACK(12);

    hsd_804D7998 = hsd_804D7984;
    blocks = state->x8 * hsd_803B2674(state);
    buf1.type = 7;
    buf1.state = PTR_TO_U32(state);
    buf1.x8 = channel;
    buf1.xC = blocks;
    result = fn_803AC168((s32*) &buf1);
    if (result < 0) {
        snap1 = hsd_804D7998;
        if (snap1 >= 0) {
            saved1 = snap1;
            while (saved1 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved1].x10 = 0;
                saved1 = (saved1 + 1) % 128;
            }
            hsd_804D7984 = snap1;
        }
        return result;
    }

    result = fn_803B0E9C(state, file_id, seq_num, 1, 1);
    if (result < 0) {
        snap2 = hsd_804D7998;
        if (snap2 >= 0) {
            saved2 = snap2;
            while (saved2 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved2].x10 = 0;
                saved2 = (saved2 + 1) % 128;
            }
            hsd_804D7984 = snap2;
        }
        return result;
    }

    result = fn_803B1338(state, 1);
    if (result < 0) {
        snap3 = hsd_804D7998;
        if (snap3 >= 0) {
            saved3 = snap3;
            while (saved3 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved3].x10 = 0;
                saved3 = (saved3 + 1) % 128;
            }
            hsd_804D7984 = snap3;
        }
        return result;
    }

    buf2.type = 8;
    buf2.state = PTR_TO_U32(state);
    result = fn_803AC168((s32*) &buf2);
    if (result < 0) {
        snap = hsd_804D7998;
        if (snap >= 0) {
            saved = snap;
            while (saved != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved].x10 = 0;
                saved = (saved + 1) % 128;
            }
            hsd_804D7984 = snap;
        }
        return result;
    }

    entries[0].x0 = 6;
    entries[0].x4 = PTR_TO_U32(state);
    entries[0].x8 = callback;
    result = 0;
    entries[0].xC = result;
    if (result < 0) {
        snap4 = hsd_804D7998;
        if (snap4 >= 0) {
            saved4 = snap4;
            while (saved4 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved4].x10 = 0;
                saved4 = (saved4 + 1) % 128;
            }
            hsd_804D7984 = snap4;
        }
        return 0;
    }
    hsd_804D7998 = -1;
    return 0;
}

s32 fn_803B21E8(s32 card_state, s32 file_id, s32 seq_num, s32 callback)
{
    CardCmd buf;
    s32 result;
    s32 saved;
    s32 snap;
    s32 saved1;
    s32 snap1;
    s32 saved2;
    s32 snap2;
    CardBufEntry* entries = (CardBufEntry*) hsd_804D1138;
    PAD_STACK(8);

    hsd_804D7998 = hsd_804D7984;
    result = fn_803B0E9C(U32_TO_PTR(CardState*, card_state), file_id, seq_num, 0, 1);
    if (result < 0) {
        snap1 = hsd_804D7998;
        if (snap1 >= 0) {
            saved1 = snap1;
            while (saved1 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved1].x10 = 0;
                saved1 = (saved1 + 1) % 128;
            }
            hsd_804D7984 = snap1;
        }
        return result;
    }

    buf.type = 8;
    buf.state = (u32) card_state;
    result = fn_803AC168((s32*) &buf);
    if (result < 0) {
        snap2 = hsd_804D7998;
        if (snap2 >= 0) {
            saved2 = snap2;
            while (saved2 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved2].x10 = 0;
                saved2 = (saved2 + 1) % 128;
            }
            hsd_804D7984 = snap2;
        }
        return result;
    }

    entries[0].x0 = 7;
    entries[0].x4 = card_state;
    entries[0].x8 = callback;
    result = 0;
    entries[0].xC = result;
    if (result < 0) {
        snap = hsd_804D7998;
        if (snap >= 0) {
            saved = snap;
            while (saved != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved].x10 = 0;
                saved = (saved + 1) % 128;
            }
            hsd_804D7984 = snap;
        }
        return 0;
    }
    hsd_804D7998 = -1;
    return 0;
}

void hsd_803B2374(void)
{
    int i;

    hsd_804D7990 = 0;
    hsd_804D7994 = 0;
    memset(&hsd_804D2348, 0, sizeof(hsd_804D2348));
    hsd_804D7980 = 0;
    hsd_804D7984 = 0;
    hsd_804D799C = 2;
    for (i = 0; i < 128; i++) {
        hsd_804D1148[i][0] = 0;
    }
    hsd_804D7988 = 0;
}

void hsd_803B24E4(s32* ctx, int channel, int file_no, void* work_buf)
{
    memset((CardState*) ctx, 0, sizeof(CardState));
    ((CardState*) ctx)->x20 = -1;
    ((CardState*) ctx)->x4 = channel;
    ((CardState*) ctx)->x8 = file_no;
    ((CardState*) ctx)->x0 = PTR_TO_U32(work_buf);
}

static inline HsdCmdEntry* hsd_803B2550_inline(u8* arg0, s32 arg1)
{
    return &((HsdCmdEntry*) (arg0 + 0x1210))[arg1];
}

int hsd_803B2550(s32* arg0, const char* arg1, void (*arg2)(int, int))
{
    s32 new_var;
    u8* base = hsd_804D1138;
    s32 chan = arg0[1];
    s32 new_var3;
    s32 new_var2;
    s32 retries;
    s32 result;
    s32 write_idx;
    new_var2 = chan;
    for (retries = 0; retries < 10; retries++) {
        result = CARDOpen(new_var2, (char*) arg1, (CARDFileInfo*) (arg0 + 3));
        if (result != -1) {
            break;
        }
    }

    if (result < 0) {
        return result;
    }

    result = 0;
    {
        s32 tmp = arg0[4];
        do {
            if (tmp != -1) {
                break;
            }
            result++;
        } while (result < 10);
        write_idx = arg0[4];
        retries = (new_var = (new_var3 = arg0[4]));
        write_idx = retries;
        if (tmp < 0) {
            return new_var;
        }
    }

    for (chan = 0; chan < 10; chan++) {
        if (CARDClose((CARDFileInfo*) (arg0 + 3)) != -1) {
            break;
        }
    }

    {
        s32 read_idx = hsd_804D7990;
        retries = hsd_804D7994;

        if (read_idx == retries) {
            if (CMD_QUEUE(base)[read_idx].type != 0) {
                return -265;
            }
        }
    }

    {
        HsdCmdEntry* entry = hsd_803B2550_inline(base, retries);
        s32 next = retries + 1;
        entry->type = 5;
        entry->f1 = PTR_TO_U32(arg0);
        entry->f2 = write_idx;
        entry->f5 = PTR_TO_U32(arg2);
        hsd_804D7994 = next % 32;
    }

    return 0;
}

s32 hsd_803B2674(CardState* state)
{
    s32 blocks;

    state->x24 = hsd_803AC340(&state->x3B0);

    blocks = (0x2F + state->x24 + state->x8) / state->x8;
    blocks += fn_803AC7DC(state);

    return blocks;
}

s32 fn_803B26CC(CardState* state, s32 file_id, s32 seq_num, s32 version,
                void (*callback)(s32, s32))
{
    s32 result;
    CardContext* context = (CardContext*) hsd_804D1138;
    PAD_STACK(8);

    state->x24 = hsd_803AC340(&state->x3B0);
    hsd_804D7998 = hsd_804D7984;

    result = queueHeaderBlocks(state, file_id, seq_num, version);

    if (result < 0) {
        if (hsd_804D7998 >= 0) {
            rollbackCardCommands(context, hsd_804D7998);
        }
        return result;
    }

    context->x0 = 5;
    context->x4 = PTR_TO_U32(state);
    context->x8 = PTR_TO_U32(callback);
    context->xC = 0;
    hsd_804D7998 = -1;
    return 0;
}
