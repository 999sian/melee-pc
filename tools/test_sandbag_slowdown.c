...
int main(void)
{
    ...
    for (int sign = -1; sign <= 1; sign += 2) {
        ...
        for (int frame = 0; frame < 8; ++frame) {
            vx = ftCommon_SandbagKnockbackDeaccel(vx, ftCommon_SandbagGetKnockbackDeaccelX(&fighter));
            vy = ftCommon_SandbagKnockbackDeaccel(vy, ftCommon_SandbagGetKnockbackDeaccelY(&fighter));
        }
        assert(vx == 0 && vy == 0);
    }
