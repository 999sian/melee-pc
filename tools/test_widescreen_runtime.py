...
p.add_argument("--mode", type=int, default=2, help="GameModeKind: 2 VS, 30 giant, 31 stamina, 10 camera")
...
# Unique per run: concurrent sweeps must not grab each other's windows.
env = dict(os.environ, SDL_VIDEO_DRIVER="x11", MELEE_WINDOW_TITLE=f"melee-wide-{out.name}",
    MELEE_VSYNC="0", MELEE_HEAP_CHECK="1", MELEE_TEST_SINGLE_SHOT="1", MELEE_TEST_WIDE_INFO="1",
    MELEE_TEST_MODE=str(a.mode),
...
ok = f"SPECIAL_TEST PASS mode={a.mode} " in s and "exited normally" in s and "received signal" not in s and "Python Exception" not in s
print(("PASS" if ok else "FAIL") + f": aspect={a.aspect} mode={a.mode} stage={a.stage}; {out/'gdb.log'}")
