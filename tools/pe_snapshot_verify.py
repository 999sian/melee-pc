#!/usr/bin/env python3
"""Fail the PE link if rollback markers do not contain required game globals."""
import argparse
import subprocess
...
def main():
    ...
    if any(end <= start for start, end in ranges):
        raise SystemExit('Windows rollback section is empty or reversed')
    if max(ranges[0][0], ranges[1][0]) < min(ranges[0][1], ranges[1][1]):
        raise SystemExit('Windows rollback sections overlap')
    for names, expected in ((args.tracked, True), (args.excluded, False)):
        for name in names:
            if name not in addresses:
                raise SystemExit(f'Windows rollback validation is missing symbol {name}')
            tracked = any(start <= addresses[name] < end for start, end in ranges)
            if tracked != expected:
                raise SystemExit(f'Windows rollback misplaced {name}: tracked={tracked}, expected={expected}')
    print('Windows rollback sections: game globals included, engine/audio globals excluded')
