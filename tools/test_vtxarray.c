...
static void indexed_nbt3(GXAttr attr, GXAttrType type)
{
    HSD_VtxDescList desc[] = {
        { .attr = attr, .attr_type = type, .comp_cnt = GX_NRM_NBT3,
          .comp_type = GX_F32, .stride = 36 },
        { .attr = GX_VA_TEX0, .attr_type = GX_INDEX8,
          .comp_cnt = GX_TEX_ST, .comp_type = GX_F32, .stride = 8 },
        { .attr = GX_VA_NULL }
    };
    ...
    const u8 dl16[] = { GX_DRAW_POINTS, 0, 2,
                       0, 1, 1, 44, 0, 2, 9, 0, 3, 0, 4, 0, 5, 7 };
    desc[0].vertex = (u32) (uintptr_t) normals;
    desc[1].vertex = (u32) (uintptr_t) texcoords;
    free(s_table);
    s_table = NULL;
    s_count = s_cap = 0;
    scan_display_list(desc, type == GX_INDEX8 ? dl8 : dl16,
                      type == GX_INDEX8 ? sizeof(dl8) : sizeof(dl16));
