...
void AuroraGetRenderSize(u32* w, u32* h) { *w = render_w; *h = render_h; }
void AuroraGetWindowSize(u32* w, u32* h) { *w = window_w; *h = window_h; }
void AuroraSetPresentationAspect(float aspect) { requested = aspect; }
...
{
    if (!window_w || !window_h || requested <= 0) { render_w = window_w; render_h = window_h; return; }
    if ((float) window_w / window_h > requested) {
        render_h = window_h; render_w = (u32) lroundf(window_h * requested);
    } else {
        render_w = window_w; render_h = (u32) lroundf(window_w / requested);
    }
}
...
int main(void)
{
    ...
    pc_widescreen_set_mode(2); pc_widescreen_set_scene(true); present();
